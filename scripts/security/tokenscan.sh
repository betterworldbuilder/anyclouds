...
ALLOW_FILE="scripts/security/tokenscan-allow.txt"
...
Output:
  ...
  -v, --verbose    Also list allowlisted suppressions and scan stats.
      --no-allow   Ignore $ALLOW_FILE (show raw, unfiltered hits).
      --no-color   Disable ANSI colour.
  -h, --help       This text.

Exit codes: 0 clean, 1 findings, 2 usage/environment error.

Allowlist: add ERE patterns to $ALLOW_FILE (matched against
"path:line:match:context"), or put the marker  tokenscan:allow  on the
...
allow_regexes() {
    [ "$USE_ALLOW" -eq 1 ] && [ -f "$ALLOW_FILE" ] || return 0
...
scan_cred_files() {
    ...
    while IFS= read -r -d '' f; do
        ...
        case "$rel" in "$SELF_REL"|"$ALLOW_FILE") continue ;; esac
...
report() {
    ...
    if [ $((nhigh + nmed)) -eq 0 ]; then
        ...
        [ "$nsup" -gt 0 ] && echo "   ${nsup} known-benign match(es) suppressed by $ALLOW_FILE."
    else
        echo "${C_RED}${C_BLD}❌ ${nhigh} high, ${nmed} medium finding(s).${C_off}"
        echo "   False positive? Add a pattern to $ALLOW_FILE"
