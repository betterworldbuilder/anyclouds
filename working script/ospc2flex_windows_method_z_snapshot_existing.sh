...
cinder_volume_raw_export() {
  ...
  log_download_status "snapwin_cinder_volume_create" "0" "$((volume_size * 1024))" "starting"
  ...
  log_download_status "snapwin_cinder_volume_create" "0" "$((volume_size * 1024))" "creating" "" "" "volume=$volume_id"
  cinder_wait_volume_status "$volume_id" "available" 3600 || return 1
  log_download_status "snapwin_cinder_volume_create" "$((volume_size * 1024))" "$((volume_size * 1024))" "available" "100.0" "0" "volume=$volume_id"

  log "[ZS3B_CINDER_VOLUME_EXPORT] attaching volume to helper/jumphost"
  log_download_status "snapwin_cinder_attach" "0" "$((volume_size * 1024))" "starting" "" "" "volume=$volume_id"
  rackspace_attach_volume "$helper_id" "$volume_id" || return 1
  cinder_wait_volume_status "$volume_id" "in-use" 900 || return 1
  log_download_status "snapwin_cinder_attach" "0" "$((volume_size * 1024))" "in-use" "" "" "volume=$volume_id"
  ...
  log_download_status "snapwin_raw_copy" "0" "$dev_mb" "starting" "0.0" "unknown"
  set +e
  ...
  while kill -0 "$dd_pid" 2>/dev/null; do
    ...
    if kill -0 "$dd_pid" 2>/dev/null; then
      ...
      log_download_status "snapwin_raw_copy" "$copied_mb" "$dev_mb" "copying" "$pct" "$eta_min"
    fi
  done
  wait "$dd_pid"; dd_rc=$?
  set -e
  ...
  log_download_status "snapwin_raw_copy" "$final_mb" "$dev_mb" "complete" "100.0" "0"
