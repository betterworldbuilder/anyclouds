...
SSH_PUB_KEY=''
FAIL_FAST=0
RESULTS_CSV='/home/dzoan/OSPC2FLEX/osflex-deployer-2.0/1342314_tenant_deploy_results.csv'
RESOURCE_MAP_CSV='/home/dzoan/OSPC2FLEX/osflex-deployer-2.0/1342314_tenant_deploy_resource_map.csv'
STEP_PASS=0
STEP_FAIL=0
...
run_step 'step-0001' 'compute' 'server' 'ospc-jumpHost' 'create_server_local_boot' 'image=Ubuntu 24.04,auth_mode=ssh_key' <<'STEP_EOF'
...
run_step 'step-0002' 'compute' 'floating_ip' 'ospc-jumpHost' 'assign_floating_ip' 'server=ospc-jumpHost,network=PUBLICNET' <<'STEP_EOF'
...
run_step 'step-0003' 'compute' 'server' 'u24-postgresl' 'create_server_local_boot' 'image=Ubuntu 24.04,auth_mode=ssh_key' <<'STEP_EOF'
...
run_step 'step-0004' 'compute' 'floating_ip' 'u24-postgresl' 'assign_floating_ip' 'server=u24-postgresl,network=PUBLICNET' <<'STEP_EOF'
...
run_step 'step-0005' 'compute' 'server' 'u24-FrontEnd' 'create_server_local_boot' 'image=Ubuntu 24.04,auth_mode=ssh_key' <<'STEP_EOF'
...
run_step 'step-0006' 'compute' 'floating_ip' 'u24-FrontEnd' 'assign_floating_ip' 'server=u24-FrontEnd,network=PUBLICNET' <<'STEP_EOF'
...
run_step 'step-0007' 'compute' 'server' 'php-ospc' 'create_server_local_boot' 'image=Rocky Linux 8,auth_mode=ssh_key' <<'STEP_EOF'
...
run_step 'step-0008' 'compute' 'floating_ip' 'php-ospc' 'assign_floating_ip' 'server=php-ospc,network=PUBLICNET' <<'STEP_EOF'
...
run_step 'step-0009' 'compute' 'server' 'Windows Server 2019Re' 'create_server_local_boot' 'image=Windows Server 2019,auth_mode=windows_password' <<'STEP_EOF'
echo "Creating server Windows Server 2019Re"
openstack server create --flavor 'gp.5.2.4' --image 'Windows Server 2019' --network "$PRIVATE_NETWORK" --security-group "$SECURITY_GROUP" --password 'lWISUaYty3E8q0' 'Windows Server 2019Re'
STEP_EOF
...
run_step 'step-0010' 'compute' 'floating_ip' 'Windows Server 2019Re' 'assign_floating_ip' 'server=Windows Server 2019Re,network=PUBLICNET' <<'STEP_EOF'
...
run_step 'step-0011' 'compute' 'server' 'win2019websql2019' 'create_server_local_boot' 'image=Windows Server 2019 with SQL 2019 Web,auth_mode=windows_password' <<'STEP_EOF'
echo "Creating server win2019websql2019"
openstack server create --flavor 'gp.5.2.4' --image 'Windows Server 2019 with SQL 2019 Web' --network "$PRIVATE_NETWORK" --security-group "$SECURITY_GROUP" --password '7fSyMlsBbLZNEm' 'win2019websql2019'
STEP_EOF
...
run_step 'step-0012' 'compute' 'floating_ip' 'win2019websql2019' 'assign_floating_ip' 'server=win2019websql2019,network=PUBLICNET' <<'STEP_EOF'
...
run_step 'step-0013' 'compute' 'server' 'Windows Server 2016 + SQL Server 2019' 'create_server_local_boot' 'image=Windows Server 2016,auth_mode=windows_password' <<'STEP_EOF'
echo "Creating server Windows Server 2016 + SQL Server 2019"
openstack server create --flavor 'gp.5.2.4' --image 'Windows Server 2016' --network "$PRIVATE_NETWORK" --security-group "$SECURITY_GROUP" --password 'cU1dynFGgU6GMr' 'Windows Server 2016 + SQL Server 2019'
STEP_EOF
...
run_step 'step-0014' 'compute' 'floating_ip' 'Windows Server 2016 + SQL Server 2019' 'assign_floating_ip' 'server=Windows Server 2016 + SQL Server 2019,network=PUBLICNET' <<'STEP_EOF'
...
run_step 'step-0015' 'compute' 'server' 'u24Backend' 'create_server_local_boot' 'image=Ubuntu 24.04,auth_mode=ssh_key' <<'STEP_EOF'
...
run_step 'step-0016' 'compute' 'floating_ip' 'u24Backend' 'assign_floating_ip' 'server=u24Backend,network=PUBLICNET' <<'STEP_EOF'
...
run_step 'step-0017' 'compute' 'server' 'HA percona 8-02' 'create_server_local_boot' 'image=Ubuntu 22.04,auth_mode=ssh_key' <<'STEP_EOF'
...
run_step 'step-0018' 'compute' 'server' 'drupal' 'create_server_local_boot' 'image=Ubuntu 22.04,auth_mode=ssh_key' <<'STEP_EOF'
...
run_step 'step-0019' 'compute' 'server' 'HA-Mysql8-01' 'create_server_local_boot' 'image=Ubuntu 22.04,auth_mode=ssh_key' <<'STEP_EOF'
...
run_step 'step-0020' 'compute' 'server' 'HA-mariaDB-02' 'create_server_local_boot' 'image=Ubuntu 22.04,auth_mode=ssh_key' <<'STEP_EOF'
...
run_step 'step-0021' 'compute' 'server' 'php-ospc_Database' 'create_server_local_boot' 'image=Ubuntu 22.04,auth_mode=ssh_key' <<'STEP_EOF'
...
run_step 'step-0022' 'compute' 'server' 'HA percona 8-03' 'create_server_local_boot' 'image=Ubuntu 22.04,auth_mode=ssh_key' <<'STEP_EOF'
...
run_step 'step-0023' 'compute' 'server' 'HA-mariaDB-03' 'create_server_local_boot' 'image=Ubuntu 22.04,auth_mode=ssh_key' <<'STEP_EOF'
...
run_step 'step-0024' 'compute' 'server' 'Stack-05_Database' 'create_server_local_boot' 'image=Ubuntu 22.04,auth_mode=ssh_key' <<'STEP_EOF'
...
run_step 'step-0025' 'compute' 'server' 'sql' 'create_server_local_boot' 'image=Ubuntu 22.04,auth_mode=ssh_key' <<'STEP_EOF'
...
run_step 'step-0026' 'compute' 'server' 'HA-Mysql8-02' 'create_server_local_boot' 'image=Ubuntu 22.04,auth_mode=ssh_key' <<'STEP_EOF'
...
run_step 'step-0027' 'compute' 'server' 'HA-mariaDB-01' 'create_server_local_boot' 'image=Ubuntu 22.04,auth_mode=ssh_key' <<'STEP_EOF'
...
run_step 'step-0028' 'compute' 'server' 'Instance-05-03' 'create_server_local_boot' 'image=Ubuntu 22.04,auth_mode=ssh_key' <<'STEP_EOF'
...
run_step 'step-0029' 'compute' 'server' 'HA percona 8-01' 'create_server_local_boot' 'image=Ubuntu 22.04,auth_mode=ssh_key' <<'STEP_EOF'
...
run_step 'step-0030' 'compute' 'server' 'Instance-05-02' 'create_server_local_boot' 'image=Ubuntu 22.04,auth_mode=ssh_key' <<'STEP_EOF'
...
run_step 'step-0031' 'compute' 'server' 'HA-Mysql8-03' 'create_server_local_boot' 'image=Ubuntu 22.04,auth_mode=ssh_key' <<'STEP_EOF'
...
run_step 'step-0032' 'storage' 'volume' 'data-u24-postgresl-1' 'create_and_attach_volume' 'server=u24-postgresl,device=/dev/vdb,size_gb=75' <<'STEP_EOF'
wait_for_server_active 'u24-postgresl'
echo "Creating data volume data-u24-postgresl-1 for u24-postgresl"
openstack volume create --size 75 --type "$VOLUME_TYPE" 'data-u24-postgresl-1'
wait_for_volume_available 'data-u24-postgresl-1'
VOL_ID=$(openstack volume show -f value -c id 'data-u24-postgresl-1')
openstack server add volume 'u24-postgresl' "$VOL_ID" --device '/dev/vdb'
STEP_EOF

# Map OSPC volume → FLEX volume
_MAP_VOL_ID=$(openstack volume show -f value -c id 'data-u24-postgresl-1' 2>/dev/null || echo "")
append_resource_map 'd9b51e0a-f7ae-4678-9e18-2971089c6af7' 'u24-postgresl' 'volume' 'data-u24-postgresl-1' "$_MAP_VOL_ID" "" "" 'created'

run_step 'step-0033' 'storage' 'volume' 'data-u24-frontend-1' 'create_and_attach_volume' 'server=u24-FrontEnd,device=/dev/vdc,size_gb=75' <<'STEP_EOF'
wait_for_server_active 'u24-FrontEnd'
echo "Creating data volume data-u24-frontend-1 for u24-FrontEnd"
openstack volume create --size 75 --type "$VOLUME_TYPE" 'data-u24-frontend-1'
wait_for_volume_available 'data-u24-frontend-1'
VOL_ID=$(openstack volume show -f value -c id 'data-u24-frontend-1')
openstack server add volume 'u24-FrontEnd' "$VOL_ID" --device '/dev/vdc'
STEP_EOF

# Map OSPC volume → FLEX volume
_MAP_VOL_ID=$(openstack volume show -f value -c id 'data-u24-frontend-1' 2>/dev/null || echo "")
append_resource_map '7c7f7240-9973-43b6-9b14-bf4969806e86' 'u24-FrontEnd' 'volume' 'data-u24-frontend-1' "$_MAP_VOL_ID" "" "" 'created'

run_step 'step-0034' 'storage' 'volume' 'data-u24-frontend-2' 'create_and_attach_volume' 'server=u24-FrontEnd,device=/dev/vdb,size_gb=75' <<'STEP_EOF'
wait_for_server_active 'u24-FrontEnd'
echo "Creating data volume data-u24-frontend-2 for u24-FrontEnd"
openstack volume create --size 75 --type "$VOLUME_TYPE" 'data-u24-frontend-2'
wait_for_volume_available 'data-u24-frontend-2'
VOL_ID=$(openstack volume show -f value -c id 'data-u24-frontend-2')
openstack server add volume 'u24-FrontEnd' "$VOL_ID" --device '/dev/vdb'
STEP_EOF

# Map OSPC volume → FLEX volume
_MAP_VOL_ID=$(openstack volume show -f value -c id 'data-u24-frontend-2' 2>/dev/null || echo "")
append_resource_map '7c7f7240-9973-43b6-9b14-bf4969806e86' 'u24-FrontEnd' 'volume' 'data-u24-frontend-2' "$_MAP_VOL_ID" "" "" 'created'

run_step 'step-0035' 'storage' 'volume' 'data-windows-server-2019re-1' 'create_and_attach_volume' 'server=Windows Server 2019Re,device=/dev/vdb,size_gb=75' <<'STEP_EOF'
wait_for_server_active 'Windows Server 2019Re'
echo "Creating data volume data-windows-server-2019re-1 for Windows Server 2019Re"
openstack volume create --size 75 --type "$VOLUME_TYPE" 'data-windows-server-2019re-1'
wait_for_volume_available 'data-windows-server-2019re-1'
VOL_ID=$(openstack volume show -f value -c id 'data-windows-server-2019re-1')
openstack server add volume 'Windows Server 2019Re' "$VOL_ID" --device '/dev/vdb'
STEP_EOF

# Map OSPC volume → FLEX volume
_MAP_VOL_ID=$(openstack volume show -f value -c id 'data-windows-server-2019re-1' 2>/dev/null || echo "")
append_resource_map 'd8753e16-0039-4c04-993c-7e14be21e212' 'Windows Server 2019Re' 'volume' 'data-windows-server-2019re-1' "$_MAP_VOL_ID" "" "" 'created'

run_step 'step-0036' 'storage' 'volume' 'data-win2019websql2019-1' 'create_and_attach_volume' 'server=win2019websql2019,device=/dev/vdb,size_gb=75' <<'STEP_EOF'
wait_for_server_active 'win2019websql2019'
echo "Creating data volume data-win2019websql2019-1 for win2019websql2019"
openstack volume create --size 75 --type "$VOLUME_TYPE" 'data-win2019websql2019-1'
wait_for_volume_available 'data-win2019websql2019-1'
VOL_ID=$(openstack volume show -f value -c id 'data-win2019websql2019-1')
openstack server add volume 'win2019websql2019' "$VOL_ID" --device '/dev/vdb'
STEP_EOF

# Map OSPC volume → FLEX volume
_MAP_VOL_ID=$(openstack volume show -f value -c id 'data-win2019websql2019-1' 2>/dev/null || echo "")
append_resource_map '47f882bf-4d6d-41d6-baef-cfcc7a9c5e61' 'win2019websql2019' 'volume' 'data-win2019websql2019-1' "$_MAP_VOL_ID" "" "" 'created'

run_step 'step-0037' 'storage' 'volume' 'data-windows-server-2016-sql-server-2019-1' 'create_and_attach_volume' 'server=Windows Server 2016 + SQL Server 2019,device=/dev/vdb,size_gb=75' <<'STEP_EOF'
wait_for_server_active 'Windows Server 2016 + SQL Server 2019'
echo "Creating data volume data-windows-server-2016-sql-server-2019-1 for Windows Server 2016 + SQL Server 2019"
openstack volume create --size 75 --type "$VOLUME_TYPE" 'data-windows-server-2016-sql-server-2019-1'
wait_for_volume_available 'data-windows-server-2016-sql-server-2019-1'
VOL_ID=$(openstack volume show -f value -c id 'data-windows-server-2016-sql-server-2019-1')
openstack server add volume 'Windows Server 2016 + SQL Server 2019' "$VOL_ID" --device '/dev/vdb'
STEP_EOF

# Map OSPC volume → FLEX volume
_MAP_VOL_ID=$(openstack volume show -f value -c id 'data-windows-server-2016-sql-server-2019-1' 2>/dev/null || echo "")
append_resource_map '741c63ea-e4ae-4048-b6cb-fc7b7716034e' 'Windows Server 2016 + SQL Server 2019' 'volume' 'data-windows-server-2016-sql-server-2019-1' "$_MAP_VOL_ID" "" "" 'created'

run_step 'step-0038' 'load_balancer' 'load_balancer' 'frontend- Load-Balancer-01' 'create_or_reuse_lb_stack' 'provider=amphora,protocol=HTTP,listener_port=80,algorithm=LEAST_CONNECTIONS' <<'STEP_EOF'
...
run_step 'step-0039' 'load_balancer' 'load_balancer_member' 'u24-FrontEnd' 'ensure_lb_pool_member' 'lb=frontend- Load-Balancer-01,pool=frontend-load-balancer-01-pool,member_port=80' <<'STEP_EOF'
wait_for_server_active 'u24-FrontEnd'
VIP_SUBNET_ID=$(openstack subnet show -f value -c id "$SUBNET_NAME")
