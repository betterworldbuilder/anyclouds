        ...
        if area == "DB":
            ...
            lines.extend([
                ...
                *_haproxy_global_lines(),
                ...
                "  balance roundrobin",
                f"  server blue_source {source_ip}:{db_port} check weight {_safe_int(item.get('blue_weight'), 100)}",
                f"  server green_flex {target_ip}:{db_port} check weight {_safe_int(item.get('green_weight'), 0)}",
                "EOF_HAPROXY_DB",
            ...
            if item.get("lb_option") == "source_lb" or item.get("cutover_method") == "blue_green_source_load_balancer":
                ...
                lines.extend([
                    ...
                    f"run_or_print bash -lc {_sh(f'openstack loadbalancer member list {_sh(pool_name)} -f value -c address 2>/dev/null | grep -Fxq {_sh(source_ip)} || openstack loadbalancer member create --name blue_source --address {_sh(source_ip)} --protocol-port {app_port} --weight {_safe_int(item.get("blue_weight"), 90)} {_sh(pool_name)}')}",
                    f"run_or_print bash -lc {_sh(f'openstack loadbalancer member list {_sh(pool_name)} -f value -c address 2>/dev/null | grep -Fxq {_sh(target_ip)} || openstack loadbalancer member create --name green_flex --address {_sh(target_ip)} --protocol-port {app_port} --weight {_safe_int(item.get("green_weight"), 10)} {_sh(pool_name)}')}",
                    f"# Initial generated split: FLEX {_safe_int(item.get('green_weight'), 10)}% / source {_safe_int(item.get('blue_weight'), 90)}%.",
                ...
                lines.extend([
                    ...
                    *_haproxy_global_lines(),
                    ...
                    f"  server blue_source {source_ip}:{app_port} check weight {_safe_int(item.get('blue_weight'), 90)}",
                    f"  server green_flex {target_ip}:{app_port} check weight {_safe_int(item.get('green_weight'), 10)}",
                    "EOF_HAPROXY",
