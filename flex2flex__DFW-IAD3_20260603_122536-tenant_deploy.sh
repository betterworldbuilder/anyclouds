...
SSH_PUB_KEY=''
FAIL_FAST=0
RESULTS_CSV='/home/dzoan/OSPC2FLEX/osflex-deployer-fullmig-5.0.0420current/flex2flex__DFW-IAD3_20260603_122536-tenant_deploy_results.csv'
RESOURCE_MAP_CSV='/home/dzoan/OSPC2FLEX/osflex-deployer-fullmig-5.0.0420current/flex2flex__DFW-IAD3_20260603_122536-tenant_deploy_resource_map.csv'
STEP_PASS=0
STEP_FAIL=0
STEP_IGNORED=0
...
openstack server create --flavor 'mo.6.4.32' --image 'Ubuntu 24.04' --network "$PRIVATE_NETWORK" --security-group "$SECURITY_GROUP" ${KEY_NAME:+--key-name "$KEY_NAME"} 'FlexDFWjumphost'
STEP_EOF
...
openstack server create --flavor 'mo.6.2.16' --image 'Ubuntu 24.04' --network "$PRIVATE_NETWORK" --security-group "$SECURITY_GROUP" ${KEY_NAME:+--key-name "$KEY_NAME"} 'Windows-Vol-Helper'
STEP_EOF
...
run_step 'step-0005' 'compute' 'server' 'ospc2flex-dbian12-20260425-2221' 'create_server_local_boot' 'image=ospc2flex-dbian12-20260425-2221,auth_mode=ssh_key' <<'STEP_EOF'
echo "Creating server ospc2flex-dbian12-20260425-2221"
openstack server create --flavor 'gp.5.4.4' --image 'ospc2flex-dbian12-20260425-2221' --network "$PRIVATE_NETWORK" --security-group "$SECURITY_GROUP" ${KEY_NAME:+--key-name "$KEY_NAME"} 'ospc2flex-dbian12-20260425-2221'
STEP_EOF
...
run_step 'step-0006' 'compute' 'floating_ip' 'ospc2flex-dbian12-20260425-2221' 'assign_floating_ip' 'server=ospc2flex-dbian12-20260425-2221,network=PUBLICNET' <<'STEP_EOF'
...
run_step 'step-0007' 'compute' 'server' 'ospc2flex-alma8-20260425-2221' 'create_server_local_boot' 'image=ospc2flex-alma8-20260425-2221,auth_mode=ssh_key' <<'STEP_EOF'
echo "Creating server ospc2flex-alma8-20260425-2221"
openstack server create --flavor 'gp.5.4.4' --image 'ospc2flex-alma8-20260425-2221' --network "$PRIVATE_NETWORK" --security-group "$SECURITY_GROUP" ${KEY_NAME:+--key-name "$KEY_NAME"} 'ospc2flex-alma8-20260425-2221'
STEP_EOF
...
run_step 'step-0008' 'compute' 'floating_ip' 'ospc2flex-alma8-20260425-2221' 'assign_floating_ip' 'server=ospc2flex-alma8-20260425-2221,network=PUBLICNET' <<'STEP_EOF'
...
run_step 'step-0009' 'compute' 'server' 'ospc2flex-rocky8-20260425-2221' 'create_server_local_boot' 'image=ospc2flex-rocky8-20260425-2221,auth_mode=ssh_key' <<'STEP_EOF'
echo "Creating server ospc2flex-rocky8-20260425-2221"
openstack server create --flavor 'gp.5.4.4' --image 'ospc2flex-rocky8-20260425-2221' --network "$PRIVATE_NETWORK" --security-group "$SECURITY_GROUP" ${KEY_NAME:+--key-name "$KEY_NAME"} 'ospc2flex-rocky8-20260425-2221'
STEP_EOF
...
run_step 'step-0010' 'compute' 'floating_ip' 'ospc2flex-rocky8-20260425-2221' 'assign_floating_ip' 'server=ospc2flex-rocky8-20260425-2221,network=PUBLICNET' <<'STEP_EOF'
...
run_step 'step-0011' 'compute' 'server' 'ospc2flex-rocky9-20260425-2221' 'create_server_local_boot' 'image=ospc2flex-rocky9-20260425-2221,auth_mode=ssh_key' <<'STEP_EOF'
echo "Creating server ospc2flex-rocky9-20260425-2221"
openstack server create --flavor 'gp.5.4.4' --image 'ospc2flex-rocky9-20260425-2221' --network "$PRIVATE_NETWORK" --security-group "$SECURITY_GROUP" ${KEY_NAME:+--key-name "$KEY_NAME"} 'ospc2flex-rocky9-20260425-2221'
STEP_EOF
...
run_step 'step-0012' 'compute' 'floating_ip' 'ospc2flex-rocky9-20260425-2221' 'assign_floating_ip' 'server=ospc2flex-rocky9-20260425-2221,network=PUBLICNET' <<'STEP_EOF'
...
run_step 'step-0013' 'compute' 'server' 'SSHED-ospc2flex-rh6-GL-20260425-1356' 'create_server_local_boot' 'image=ospc2flex-rh6-GL-20260425-1356,auth_mode=ssh_key' <<'STEP_EOF'
echo "Creating server SSHED-ospc2flex-rh6-GL-20260425-1356"
openstack server create --flavor 'gp.5.4.4' --image 'ospc2flex-rh6-GL-20260425-1356' --network "$PRIVATE_NETWORK" --security-group "$SECURITY_GROUP" ${KEY_NAME:+--key-name "$KEY_NAME"} 'SSHED-ospc2flex-rh6-GL-20260425-1356'
STEP_EOF

# Map OSPC server → FLEX server
_MAP_FLEX_ID=$(openstack server show -f value -c id 'SSHED-ospc2flex-rh6-GL-20260425-1356' 2>/dev/null || echo "")
_MAP_FLEX_PRIV=$(instance_ip_on_network 'SSHED-ospc2flex-rh6-GL-20260425-1356' "$PRIVATE_NETWORK" 2>/dev/null || echo "")
_MAP_FLEX_FLOAT=$(openstack floating ip list --server 'SSHED-ospc2flex-rh6-GL-20260425-1356' -f value -c 'Floating IP Address' 2>/dev/null | head -n1 || echo "")
_MAP_STATUS="created"
[ -z "$_MAP_FLEX_ID" ] && _MAP_STATUS="failed"
append_resource_map 'f26fd003-e391-4181-bc49-7d2d1563bfaf' 'SSHED-ospc2flex-rh6-GL-20260425-1356' 'server' 'SSHED-ospc2flex-rh6-GL-20260425-1356' "$_MAP_FLEX_ID" "$_MAP_FLEX_PRIV" "$_MAP_FLEX_FLOAT" "$_MAP_STATUS"

run_step 'step-0014' 'compute' 'floating_ip' 'SSHED-ospc2flex-rh6-GL-20260425-1356' 'assign_floating_ip' 'server=SSHED-ospc2flex-rh6-GL-20260425-1356,network=PUBLICNET' <<'STEP_EOF'
wait_for_server_active 'SSHED-ospc2flex-rh6-GL-20260425-1356'
if server_has_floating_ip 'SSHED-ospc2flex-rh6-GL-20260425-1356'; then
  echo "Server SSHED-ospc2flex-rh6-GL-20260425-1356 already has a floating IP; skipping assignment."
else
  assign_floating_ip 'SSHED-ospc2flex-rh6-GL-20260425-1356' 'PUBLICNET'
fi
STEP_EOF
...
run_step 'step-0016' 'load_balancer' 'load_balancer' 'backend' 'create_or_reuse_lb_stack' 'provider=ovn,protocol=HTTP,listener_port=80,algorithm=ROUND_ROBIN' <<'STEP_EOF'
...
run_step 'step-0017' 'load_balancer' 'load_balancer' 'u24backend' 'create_or_reuse_lb_stack' 'provider=ovn,protocol=HTTP,listener_port=80,algorithm=ROUND_ROBIN' <<'STEP_EOF'
