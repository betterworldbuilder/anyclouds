...
This tool is for the **image portability / first-boot remediation** stage of an OSPC â†’ FLEX migration.
...
### Option A â€” Build FLEX-ready image only
...
### Option B â€” Build image + boot FLEX test VM
...
| 2 | Stage 3 | Export snapshot to same-region Cloud Files | Snapshot ID + RAX auth token | VHD/object in `ospc2flex-export` container | `POST /v2/tasks type=export` â†’ poll until `success` |
...
| 5 | Stage 4.5 | Offline repair (fstab + netplan) | `.qcow2` mounted via `qemu-nbd` | fstab cleaned, DHCP netplan written, cloud-init reset | `nbd mount` â†’ `sed fstab` â†’ write `99-ospc2flex.yaml` |
...
| 7 | Stage 4.7 | Pre-upload verification | Repaired `.qcow2` | Verify network config + fstab are correct | `nbd mount` â†’ check netplan/ifcfg + fstab |
...
| 9 | Stage 6 | Boot test VM + verify SSH | FLEX image ID + flavor + network | Running FLEX VM, SSH reachable | `openstack server create` â†’ `wait_for_ssh` |

For fastest snapshot migrations, place the jumphost in the same OSPC region as the origin VM/snapshot. For example, OSPC IAD VM â†’ OSPC IAD Glance snapshot â†’ OSPC IAD Cloud Files â†’ OSPC IAD jumphost â†’ convert/repair â†’ FLEX IAD Glance.
...
 - - - 
 
 d'þ  M a d e   w i t h   L o v e   b y   =ØdÜ  * * D z o a n . n g u y e n @ R a c k s p a c e . c o m * *   u s i n g   =ØdÜ  * * b r i a n . a b s h i e r @ R A C K S P A C E . C O M * *   a w e s o m e   =ØàÞþ  * * O S F L E X   T o p o l o g y   B u i l d e r * *   t o o l .  
 