...
if [ -f "$REPAIR_MARKER" ]; then
  ...
  if [ -f "$REPAIR_SCRIPT" ]; then
    ...
    # CentOS/RHEL LAN guardrail (mig_worker_v4 verbatim)
    case "${OS_TYPE,,}" in
      centos*|rhel7*|rhel6*)
        if [ -f "$REPAIR_LOG" ] \
          ...
          log "[LS5] ERROR [REPAIR-LAN-E5] CentOS/RHEL LAN repair markers missing"
          log "[LS5]   Required: 'Wrote fresh ifcfg-eth0 ...' and 'Enabled network.service'"
          fail_exit "LS5_OFFLINE_REPAIR" "REPAIR_LAN_E5: CentOS/RHEL LAN markers missing — see $REPAIR_LOG"
        fi ;;
    esac
    sudo qemu-nbd --disconnect "$NBD_DEV" 2>/dev/null || true
  else
