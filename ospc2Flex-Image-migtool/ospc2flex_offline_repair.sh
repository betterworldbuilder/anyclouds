...
RESOLV_EOF
      ...
      if [ "${OS_MAJOR:-0}" -le 6 ] && [ "$OS_ID" = "centos" -o "$OS_ID" = "rhel" ]; then
        # Write fresh ifcfg-eth0 for RHEL/CentOS 6 SysV mode — NO HWADDR (Xen MAC won't match virtio)
        sudo tee "$IFCFG_DIR/ifcfg-eth0" >/dev/null <<'IFCFG6_EOF'
DEVICE=eth0
BOOTPROTO=dhcp
ONBOOT=yes
NM_CONTROLLED=no
...
TYPE=Ethernet
IFCFG6_EOF
        PASS "Wrote fresh ifcfg-eth0 (no HWADDR, ONBOOT=yes, DHCP, NM_CONTROLLED=no — RHEL 6 SysV)"
...
FLEX_DS_OLD_EOF
    ...
    # Disable cloud-init network configuration entirely.
    # cloud-init init-local runs before NetworkManager and with no reachable datasource
    # (169.254.169.254 requires network to be up) it applies a "None" network config
    # that leaves eth0 DOWN. NetworkManager reads ifcfg-eth0 (ONBOOT=yes, BOOTPROTO=dhcp)
    # and is the sole network manager — cloud-init must not interfere.
    sudo tee "$MNT/etc/cloud/cloud.cfg.d/99-disable-network-config.cfg" >/dev/null <<'FLEX_NET_EOF'
# Written by ospc2flex_offline_repair.sh — NetworkManager owns networking via ifcfg-eth0
network: {config: disabled}
FLEX_NET_EOF
    PASS "cloud-init network config disabled (NetworkManager handles eth0 via ifcfg-eth0)"
