...
repaired_path="$workdir/test-dry-vm-snap-20260410130548-repaired.qcow2"
converted_path="$workdir/test-dry-vm-snap-20260410130548.qcow2"
...
img_path="$workdir/test-dry-vm-snap-20260410130548.img"
...
cleaned=0
SNAP_PREFIX="test-dry-vm-snap-20260410130548"
...
RAW_IMG="$workdir/test-dry-vm-snap-20260410130548.img"
...
NM_CONTROLLED=yes
IFCFG_EOF
          # Disable NetworkManager for CentOS 7 if network service is present
          sudo chroot "$MNT" bash -c 'systemctl enable network 2>/dev/null || true'
...
# Runs only if Stage 4.5 repair_ok=0 (NBD failed, mount failed, unknown OS, etc.)
# Uses ospc2flex_offline_repair.sh — 4-method OS detection, proven standalone script
if [ $repair_ok -eq 0 ]; then
  stage_start '4.6' 'Standalone Repair Fallback' 'Running ospc2flex_offline_repair.sh as safety net'
...
sed -i 's/$//' /tmp/ospc2flex_flex.sh  # Ensure Windows formatting doesn't corrupt variable exports
...
IMG_ID=$(openstack image create \
  ...
  --format value -c id \
  "test-dry-vm-snap-20260410130548-flex" 2>&1 || true)
