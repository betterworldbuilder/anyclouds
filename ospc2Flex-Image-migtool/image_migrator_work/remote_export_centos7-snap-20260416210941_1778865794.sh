...
if [ "$IS_WINDOWS" = "1" ]; then
  ...
  for _wf in "$workdir/centos7-snap-20260416210941".*; do
    [ -f "$_wf" ] && rm -f "$_wf" && log "  [DEL] $_wf" || true
  done
...
if [ $success -eq 0 ] && [ "1" = "1" ] && [ -x "$GLANCE_BRIDGE" ]; then
  ...
  set +e
  BRIDGE_DOWNLOAD_OUT=$(bash "$GLANCE_BRIDGE" download       --ospc-openrc /tmp/ospc2flex_ospc.sh       --image-id 477f0792-e508-4ab7-a458-fe67c05ed8fd       --dest "$img_path"       --container ospc2flex-export       --prefer-cloud-files       --retries "$export_retries"       --retry-wait "$export_retry_wait"       --min-bytes 1048576 2>&1)
  BRIDGE_DOWNLOAD_RC=$?
  set -e
...
rm -f "$img_path"
if openstack image save --file "$img_path" "477f0792-e508-4ab7-a458-fe67c05ed8fd" 2>/tmp/ospc_img_save_err.txt; then
  size=$(stat -c%s "$img_path" 2>/dev/null || echo 0)
...
REPAIR_LOG="$workdir/centos7-snap-20260416210941.repair.log"
...
if [ "1" = "1" ] && [ -x "$GLANCE_BRIDGE" ]; then
  ...
  if BRIDGE_UPLOAD_OUT=$(bash "$GLANCE_BRIDGE" upload       --flex-openrc /tmp/ospc2flex_flex.sh       --image-file "$repaired_path"       --image-name centos7-snap-20260416210941-flex       --disk-format qcow2       --container-format bare       --visibility private       --container ospc2flex-staging 2>&1); then
...
if [ -z "$IMG_ID" ] || echo "$IMG_ID" | grep -qiE 'error|failed|traceback|exception|unauthorized'; then
  ...
  IMG_ID=$(openstack image create \
    ...
    --format value -c id \
    "centos7-snap-20260416210941-flex" 2>&1 || true)
fi
if [ -z "$IMG_ID" ] || echo "$IMG_ID" | grep -qiE 'error|failed|traceback|exception|unauthorized'; then
  stage_fail 5 "Image upload produced no FLEX image ID after all methods: $IMG_ID"
fi
log "  [OK] Upload complete — Image ID: $IMG_ID"
SHOW_NAME=$(openstack image show "$IMG_ID" -f value -c name 2>/dev/null || echo "centos7-snap-20260416210941-flex")
