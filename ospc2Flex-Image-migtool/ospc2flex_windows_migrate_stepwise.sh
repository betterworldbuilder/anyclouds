...
if [ -f "$WIN_REPAIR" ]; then
  ...
  _xen_purge_flag="--purge-xen"
  if [ "${OSPC2FLEX_WIN_PURGE_XEN:-1}" != "1" ]; then
    _xen_purge_flag="--no-purge-xen"
  fi
  _bf_flag=()
  if [ "${OSPC2FLEX_WINDOWS_MODE:-}" = "bruteforce_flex" ]; then
    _bf_flag=(--bruteforce-flex)
  fi
  if [ "${OSPC2FLEX_VIRTIO_ISO_FORCE_OFFLINE:-0}" = "1" ]; then
    bash "$WIN_REPAIR" --qcow2 "$QCOW" --force "$_xen_purge_flag" "${_bf_flag[@]}" --debug --debug-log "$_win_dbg"
  else
    env OSPC2FLEX_VIRTIO_ISO_OFFLINE=0 OSPC2FLEX_VIRTIO_ISO_NO_DOWNLOAD=0 \
      bash "$WIN_REPAIR" --qcow2 "$QCOW" --force "$_xen_purge_flag" "${_bf_flag[@]}" --debug --debug-log "$_win_dbg"
  fi
  _repair_rc=$?
  set -e
