...
if [ -s "$QCOW" ] && qemu-img info "$QCOW" >/dev/null 2>&1 && [ "$(stat -c%s "$QCOW" 2>/dev/null || echo 0)" -ge 1048576 ]; then
  ok "Resume from existing qcow2: $QCOW — skipping steps 1-3"
  ...
  if [ -f "$WIN_REPAIR" ]; then
    ...
    _repair_rc=$?
    set -e
    [ "$_repair_rc" -eq 0 ] || die "Windows repair failed (rc=$_repair_rc)"
  ...
  source "$FLEX_CREDS"
  FLEX_IMG_ID="$(openstack image create "$LABEL" --disk-format qcow2 --container-format bare --file "$QCOW" --private -f value -c id | tr -d '\r\n')"
  [ -n "$FLEX_IMG_ID" ] || die "FLEX image upload failed"
  ok "FLEX image ID: $FLEX_IMG_ID"
  log "Step 6/6: Boot FLEX VM from migrated image"
  _sw_boot_args=(server create "$LABEL" --image "$FLEX_IMG_ID" --flavor "$FLAVOR" --network "$NETWORK" --wait -f value -c id)
  [ -n "$KEYPAIR" ] && _sw_boot_args+=(--key-name "$KEYPAIR")
  VM_ID="$(openstack "${_sw_boot_args[@]}" | tr -d '\r\n')"
  [ -n "$VM_ID" ] || die "FLEX VM boot failed"
  ok "FLEX VM ID: $VM_ID"
  echo "=============================================================="
  echo "DONE (resume from qcow2)"
  echo "FLEX Image  : $FLEX_IMG_ID"
  echo "FLEX VM     : $VM_ID"
  echo "=============================================================="
  exit 0
elif [ -s "$IMG" ] && qemu-img info "$IMG" >/dev/null 2>&1 && [ "$(stat -c%s "$IMG" 2>/dev/null || echo 0)" -ge 1048576 ]; then
  ok "Resume from existing raw image: $IMG — skipping steps 1-2, converting to qcow2"
