...
REPAIR_REPORT_FILE=""
CLOUDBOOT_BUNDLE=""
CLOUDBOOT_FIRSTBOOT_STAGED=0
RESTORE_REGISTRY_BACKUP=0
VIRTIO_ISO_URL="https://fedorapeople.org/groups/virt/virtio-win/direct-downloads/stable-virtio/virtio-win.iso"
VIRTIO_ISO="/tmp/virtio-win.iso"
VIRTIO_MNT="/tmp/virtio_iso_mnt"
MNT="/tmp/mnt_windows_repair_$$"
NETKVM_PRESENT=0
STRICT_NETKVM_OFFLINE="${OSPC2FLEX_STRICT_NETKVM_OFFLINE:-0}"
REQUIRED_IO_ERROR=0
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --restore-registry-backup) RESTORE_REGISTRY_BACKUP=1; shift ;;
    --cloudboot-bundle) CLOUDBOOT_BUNDLE="$2"; shift 2 ;;
    -h|--help)
      echo "Usage: $0 --qcow2 <path> [--nbd-dev /dev/nbdX] [--force] [--dry-run] [--debug] [--debug-log FILE] [--debug-trace] [--restore-registry-backup]"
      exit 0 ;;
...
# Disconnect NBD/umount (used by normal cleanup and by hive-corruption abort).
_ospc2flex_disconnect_image() {
...
# After registry backup exists: restore SYSTEM, unmount, move qcow2 to bad_artifacts/, strip sentinel.
registry_repair_abort_with_quarantine() {
  local reason="${1:-WINDOWS_SYSTEM_REGISTRY_HIVE_CORRUPTED}"
  local sys="${HIVE_SYSTEM:-$MNT/Windows/System32/config/SYSTEM}"
  if [ "$DRY_RUN" -eq 1 ]; then
    FAIL "status=WINDOWS_SYSTEM_HIVE_INVALID failure_reason=${reason} next_action=Use fresh source export or restore SYSTEM hive backup."
    exit 1
  fi
  if [ -f "${sys}.ospc2flex.bak" ]; then
    WARN "Restoring SYSTEM hive from .ospc2flex.bak before quarantine"
    sudo cp -a "${sys}.ospc2flex.bak" "$sys" 2>/dev/null || true
    sync
    validate_registry_hive "$sys" "SYSTEM" || FAIL "SYSTEM hive still unreadable after restore (artifact unusable)"
  fi
  trap - EXIT
  echo ""
  echo "── Cleanup (registry abort) ─────────────────────────────────────────────────"
  _ospc2flex_disconnect_image
  INFO "Cleanup done"
  local orig="$QCOW2"
  rm -f "${orig}.win_repaired" 2>/dev/null || true
  if [ -f "$orig" ]; then
    local bad dir
    dir="$(dirname "$orig")"
    bad="$dir/bad_artifacts"
    mkdir -p "$bad"
    local dest="$bad/$(basename "$orig").bad-registry.$(date +%Y%m%d-%H%M%S)"
    mv "$orig" "$dest" 2>/dev/null || WARN "Could not quarantine qcow2 (still at $orig)"
  fi
  FAIL "status=WINDOWS_SYSTEM_HIVE_INVALID failure_reason=${reason} next_action=Use fresh source export or restore SYSTEM hive backup."
  exit 1
}

# Swap validated SYSTEM.work over SYSTEM; on failure restore .ospc2flex.bak and quarantine.
commit_system_work_hive() {
  local config_dir="$1"
  validate_registry_hive "$config_dir/SYSTEM.work" "SYSTEM.work" || { sudo rm -f "$config_dir/SYSTEM.work" 2>/dev/null || true; return 1; }
  sudo rm -f "$config_dir/SYSTEM.pre_ospc2flex" 2>/dev/null || true
  sudo mv "$config_dir/SYSTEM" "$config_dir/SYSTEM.pre_ospc2flex"
  sudo mv "$config_dir/SYSTEM.work" "$config_dir/SYSTEM"
  sync
  if ! validate_registry_hive "$config_dir/SYSTEM" "SYSTEM"; then
    echo "ERROR: SYSTEM hive corrupted after swap. Restoring backup." >&2
    sudo cp -a "$config_dir/SYSTEM.ospc2flex.bak" "$config_dir/SYSTEM" 2>/dev/null || true
    sync
    sudo rm -f "$config_dir/SYSTEM.pre_ospc2flex" "$config_dir/SYSTEM.work" 2>/dev/null || true
    if ! validate_registry_hive "$config_dir/SYSTEM" "SYSTEM"; then
      HIVE_SYSTEM="$config_dir/SYSTEM"
      registry_repair_abort_with_quarantine "WINDOWS_SYSTEM_REGISTRY_HIVE_CORRUPTED"
    fi
    HIVE_SYSTEM="$config_dir/SYSTEM"
    registry_repair_abort_with_quarantine "WINDOWS_SYSTEM_REGISTRY_HIVE_CORRUPTED"
  fi
...
merge_registry_patch() {
  local hive="$1" prefix="$2" flat_reg="$3" out_reg reged_out reged_rc target_hive
...
PY
  target_hive="$hive"
  if [ "$(basename "$hive")" = "SYSTEM" ]; then
    local config_dir work_hive
    config_dir="$(dirname "$hive")"
    work_hive="$config_dir/SYSTEM.work"
    if [ ! -f "$config_dir/SYSTEM.ospc2flex.bak" ]; then
      sudo cp -a "$hive" "$config_dir/SYSTEM.ospc2flex.bak"
    fi
    validate_registry_hive "$hive" "SYSTEM" || { rm -f "$out_reg"; return 1; }
    sudo cp -a "$hive" "$work_hive"
    target_hive="$work_hive"
  fi
  reged_rc=0
  reged_out=$(printf 'y\n' | sudo reged -I "$target_hive" "$prefix" "$out_reg" 2>&1) || reged_rc=$?
  printf '%s\n' "$reged_out"
  rm -f "$out_reg"
  ...
  if [ "$reged_rc" -eq 0 ] || { echo "$reged_out" | grep -q "operation SUCCEEDED" && echo "$reged_out" | grep -q " - OK"; }; then
    if [ "$(basename "$hive")" = "SYSTEM" ]; then
      local config_dir work_hive
      config_dir="$(dirname "$hive")"
      work_hive="$config_dir/SYSTEM.work"
      validate_registry_hive "$work_hive" "SYSTEM.work" || { sudo rm -f "$work_hive" 2>/dev/null || true; return 1; }
      HIVE_SYSTEM="$hive"
      commit_system_work_hive "$config_dir" || return 1
    fi
    return 0
  fi
  if [ "$(basename "$hive")" = "SYSTEM" ]; then
    sudo rm -f "$(dirname "$hive")/SYSTEM.work" 2>/dev/null || true
  fi
  return "$reged_rc"
}
...
validate_registry_hive() {
  local hive="$1" name="$2" test_out test_err rc rc_hivex rc_reged
  local hive_size
  if [ ! -s "$hive" ]; then
    echo "ERROR: $name hive missing or empty: $hive" >&2
    return 1
  fi
  hive_size=$(stat -c%s "$hive" 2>/dev/null || echo 0)
  if [ "${hive_size:-0}" -lt 1048576 ]; then
    echo "ERROR: $name hive too small to be valid (${hive_size} bytes): $hive" >&2
    return 1
  fi
  test_out="/tmp/${name}.regtest.$$"
  test_err="/tmp/${name}.regtest.err.$$"
  rc=1
  rc_hivex=1
  rc_reged=1
  if command -v hivexregedit >/dev/null 2>&1; then
    sudo hivexregedit --export "$hive" '\' >"$test_out" 2>"$test_err"
    rc_hivex=$?
  elif command -v hivexsh >/dev/null 2>&1; then
    sudo hivexsh "$hive" <<'EOF' >"$test_out" 2>"$test_err"
ls
EOF
    rc_hivex=$?
  fi

  # Some hosts/filesystems can make hivexsh return "Operation not supported"
  # even when the hive is readable by reged. Try reged export as secondary
  # validation before declaring corruption.
  if command -v reged >/dev/null 2>&1; then
    rc_reged=0
    case "$(basename "$hive")" in
      SOFTWARE)
        sudo reged -x "$hive" 'HKEY_LOCAL_MACHINE\SOFTWARE' '\Microsoft\Windows NT\CurrentVersion' >"$test_out" 2>"$test_err" || rc_reged=$?
        ;;
      *)
        sudo reged -x "$hive" 'HKEY_LOCAL_MACHINE\SYSTEM' '\ControlSet001' >"$test_out" 2>"$test_err" || rc_reged=$?
        ;;
    esac
    [ "${rc_reged:-0}" -eq 0 ] && rc=0
  fi
  if [ "${rc_hivex:-1}" -eq 0 ]; then
    rc=0
  fi

  if [ "$rc" -ne 0 ]; then
    echo "ERROR: $name hive validation failed: $hive" >&2
    cat "$test_err" >&2 || true
...
backup_registry_hives() {
  local config_dir="$MNT/Windows/System32/config"
  local backup_dir="$MNT/ospc2flex/registry-backups/$(date +%Y%m%d-%H%M%S)"
  local hive
  sudo mkdir -p "$backup_dir"
  for hive in SYSTEM SOFTWARE SECURITY SAM DEFAULT; do
    if [ ! -f "$config_dir/$hive" ] || [ ! -s "$config_dir/$hive" ]; then
      FAIL "Required registry hive missing or empty: $config_dir/$hive"
      return 1
    fi
    sudo cp -a "$config_dir/$hive" "$backup_dir/$hive"
    sudo cp -a "$config_dir/$hive" "$config_dir/${hive}.ospc2flex.bak"
  done
  sync
  PASS "Registry hives backed up under C:\\ospc2flex\\registry-backups and *.ospc2flex.bak"
}

run_registry_edit_atomically() {
  local reg_file="$1"
  local config_dir="$MNT/Windows/System32/config"
  local hive_system="$config_dir/SYSTEM"
  local work_hive="$config_dir/SYSTEM.work"
  local rc
  validate_registry_hive "$hive_system" "SYSTEM" || return 1
  sudo rm -f "$work_hive" "$config_dir/SYSTEM.pre_ospc2flex" 2>/dev/null || true
  sudo cp -a "$hive_system" "$work_hive"
  rc=0
  merge_registry_patch "$work_hive" "HKEY_LOCAL_MACHINE\\SYSTEM" "$reg_file" 2>&1 || rc=$?
  [ "$rc" -eq 0 ] || { sudo rm -f "$work_hive" 2>/dev/null || true; return "$rc"; }
  validate_registry_hive "$work_hive" "SYSTEM.work" || { sudo rm -f "$work_hive" 2>/dev/null || true; return 1; }
...
invalidate_qcow_artifact() {
  local ts invalid_path
  ts="$(date +%Y%m%d-%H%M%S)"
  invalid_path="${QCOW2}.invalid.${ts}"
  touch "${QCOW2}.io_error" 2>/dev/null || true
  if mv "$QCOW2" "$invalid_path" 2>/dev/null; then
    WARN "Marked qcow2 invalid after required I/O error: $invalid_path"
  else
    WARN "Could not rename qcow2; leaving marker only: ${QCOW2}.io_error"
  fi
}
...
_win_fs_type=$(lsblk -rno FSTYPE "$WIN_PART" 2>/dev/null | head -1 || true)
if [ "${_win_fs_type,,}" != "ntfs" ]; then
  FAIL "WINDOWS_PARTITION_DETECTION_FAILED: expected NTFS partition, got '${_win_fs_type:-unknown}' on $WIN_PART"
  exit 1
fi
if [ ! -f "$MNT/Windows/System32/config/SYSTEM" ] || \
   [ ! -f "$MNT/Windows/System32/config/SOFTWARE" ] || \
   [ ! -f "$MNT/Windows/System32/ntoskrnl.exe" ]; then
  FAIL "WINDOWS_PARTITION_DETECTION_FAILED: missing Windows kernel/config hives on mounted partition $WIN_PART"
  exit 1
fi
if [ ! -s "$MNT/Windows/System32/config/SYSTEM" ]; then
  FAIL "WINDOWS_PARTITION_DETECTION_FAILED: SYSTEM hive is zero-byte on mounted partition $WIN_PART"
  exit 1
fi

if [ "$RESTORE_REGISTRY_BACKUP" -eq 0 ]; then
  INFO "Preflight: validating SYSTEM and SOFTWARE hives are readable (before any offline registry edits)"
  validate_registry_hive "$MNT/Windows/System32/config/SYSTEM" "SYSTEM" || {
    FAIL "WINDOWS_SYSTEM_HIVE_INVALID: SYSTEM hive failed validation before offline repair (preflight)"
    exit 1
  }
  validate_registry_hive "$MNT/Windows/System32/config/SOFTWARE" "SOFTWARE" || {
    FAIL "WINDOWS_SYSTEM_HIVE_INVALID: SOFTWARE hive failed validation before offline repair (preflight)"
    exit 1
  }
fi

if [ "$RESTORE_REGISTRY_BACKUP" -eq 1 ]; then
  echo ""
  echo "── Restore Registry Backup ───────────────────────────────────────────────"
  CONFIG_DIR="$MNT/Windows/System32/config"
  RESTORE_SRC=""
  _latest_backup="$(ls -dt "$MNT/ospc2flex/registry-backups/"* 2>/dev/null | head -1 || true)"
  if [ -n "$_latest_backup" ] && [ -f "$_latest_backup/SYSTEM" ]; then
    RESTORE_SRC="$_latest_backup/SYSTEM"
  elif [ -f "$CONFIG_DIR/SYSTEM.ospc2flex.bak" ]; then
    RESTORE_SRC="$CONFIG_DIR/SYSTEM.ospc2flex.bak"
  fi
  if [ -z "$RESTORE_SRC" ]; then
    FAIL "No registry backup source found (expected C:\\ospc2flex\\registry-backups or SYSTEM.ospc2flex.bak)"
    exit 1
  fi
  sudo cp -a "$RESTORE_SRC" "$CONFIG_DIR/SYSTEM"
  sync
  validate_registry_hive "$CONFIG_DIR/SYSTEM" "SYSTEM" || { FAIL "Restored SYSTEM hive failed validation"; exit 1; }
  PASS "SYSTEM hive restored from backup: $RESTORE_SRC"
  exit 0
fi
...
refresh_virtio_iso_mount() {
  local force_redownload="${1:-0}"
  sudo umount "$VIRTIO_MNT" 2>/dev/null || true
  sudo mkdir -p "$VIRTIO_MNT"
  ...
  if sudo mount -o loop,ro "$VIRTIO_ISO" "$VIRTIO_MNT" 2>/dev/null; then
    PASS "VirtIO ISO remounted successfully"
    return 0
  fi
  WARN "VirtIO ISO remount failed; refreshing ISO cache..."
  rm -f "$VIRTIO_ISO"
  if ! fetch_iso "$VIRTIO_ISO_URL"; then
    fetch_iso "$VIRTIO_FALLBACK_URL" || return 1
  fi
  return 0
}

copy_iso_file_with_retry() {
  local src="$1" dst="$2" label="$3"
  local fatal="${4:-1}"
  local cp_rc cp_err
  cp_err=$(mktemp)
  set +e
  sudo cp -f "$src" "$dst" 2>"$cp_err"
  cp_rc=$?
  set -e
  if [ "$cp_rc" -eq 0 ]; then
    rm -f "$cp_err"
  ...
  if grep -qi "Input/output error" "$cp_err" && [ "$fatal" = "1" ]; then
    REQUIRED_IO_ERROR=1
  fi
  rm -f "$cp_err"
  WARN "$label copy failed (possible transient ISO I/O). Retrying after remount..."
  refresh_virtio_iso_mount 0 || {
    [ "$fatal" = "1" ] && return 1 || return 0
  }
  cp_err=$(mktemp)
  set +e
  sudo cp -f "$src" "$dst" 2>"$cp_err"
  ...
  rm -f "$cp_err"
  WARN "$label copy still failing; forcing ISO re-download and retry..."
  refresh_virtio_iso_mount 1 || {
    [ "$fatal" = "1" ] && return 1 || return 0
  }
  cp_err=$(mktemp)
  set +e
  ...
  if [ "$fatal" = "1" ]; then
    ...
    WARN "$label copy failed after retries — continuing (non-fatal)"
    return 0
  fi
}
...
DRIVER_STAGE="$MNT/ospc2flex/drivers"
NETKVM_STAGE="$DRIVER_STAGE/NetKVM/$WIN_DRIVER_DIR/amd64"
...
EOF
  ...
  # Check if VirtIO was already installed via MSI on the live source VM.
  # If the DriverStore has a proper viostor .inf (MSI installs into FileRepository with catalog
  # and .inf — not just a raw sys copy), skip file injection to avoid overwriting the better version.
  _VIOSTOR_ALREADY_INSTALLED=0
  _viostor_ds_inf=$(sudo find "$DRIVERSTORE" -maxdepth 2 -name "viostor.inf" 2>/dev/null | head -1 || true)
  if [ -n "$_viostor_ds_inf" ]; then
    _viostor_ds_dir="$(dirname "$_viostor_ds_inf")"
    # MSI installs include a .cat catalog — raw offline copy does not
    if sudo find "$_viostor_ds_dir" -maxdepth 1 -name "*.cat" 2>/dev/null | grep -q .; then
      _VIOSTOR_ALREADY_INSTALLED=1
      PASS "viostor already MSI-installed in DriverStore ($( basename "$_viostor_ds_dir" )) — skipping file injection"
      printf '%s\n' "Driver.viostor.copy=SKIPPED_MSI_INSTALLED" | sudo tee -a "$REPAIR_REPORT_FILE" >/dev/null
    fi
  fi

  # --- viostor (block/disk — CRITICAL) ---
  if [ "$_VIOSTOR_ALREADY_INSTALLED" -eq 0 ] && [ -f "$VIOSTOR_SRC/viostor.sys" ]; then
    copy_iso_file_with_retry "$VIOSTOR_SRC/viostor.sys" "$DRIVERS_DIR/" "viostor.sys"
  ...
  _VIOSCSI_ALREADY_INSTALLED=0
  _vioscsi_ds_inf=$(sudo find "$DRIVERSTORE" -maxdepth 2 -name "vioscsi.inf" 2>/dev/null | head -1 || true)
  if [ -n "$_vioscsi_ds_inf" ] && sudo find "$(dirname "$_vioscsi_ds_inf")" -maxdepth 1 -name "*.cat" 2>/dev/null | grep -q .; then
    _VIOSCSI_ALREADY_INSTALLED=1
    PASS "vioscsi already MSI-installed in DriverStore — skipping file injection"
    printf '%s\n' "Driver.vioscsi.copy=SKIPPED_MSI_INSTALLED" | sudo tee -a "$REPAIR_REPORT_FILE" >/dev/null
  fi
  if [ "$_VIOSCSI_ALREADY_INSTALLED" -eq 0 ]; then
    if [ -d "$VIOSCSI_SRC" ] && [ -f "$VIOSCSI_SRC/vioscsi.sys" ]; then
      copy_iso_file_with_retry "$VIOSCSI_SRC/vioscsi.sys" "$DRIVERS_DIR/" "vioscsi.sys"
  ...
  if [ -d "$NETKVM_SRC" ] && [ -f "$NETKVM_SRC/netkvm.sys" ]; then
    sudo mkdir -p "$NETKVM_STAGE" 2>/dev/null || true
    sudo cp -f "$NETKVM_SRC/"* "$NETKVM_STAGE/" 2>/dev/null || true
    copy_iso_file_with_retry "$NETKVM_SRC/netkvm.sys" "$DRIVERS_DIR/" "netkvm.sys" 0
    sudo cp -f "$NETKVM_SRC/netkvm.inf" "$DRIVERS_DIR/" 2>/dev/null || true
    # NetKVM is non-boot-critical; DriverStore staging must never fail the run.
    sudo mkdir -p "$DRIVERSTORE/netkvm.inf_amd64" 2>/dev/null || true
    sudo cp -f "$NETKVM_SRC/"* "$DRIVERSTORE/netkvm.inf_amd64/" 2>/dev/null || true
    if [ -f "$DRIVERS_DIR/netkvm.sys" ]; then
      NETKVM_PRESENT=1
      ...
      NETKVM_PRESENT=0
      if [ "$STRICT_NETKVM_OFFLINE" = "1" ]; then
        FAIL "netkvm.sys offline copy failed and OSPC2FLEX_STRICT_NETKVM_OFFLINE=1"
        printf '%s\n' "Driver.netkvm.stage=FAIL_STRICT" | sudo tee -a "$REPAIR_REPORT_FILE" >/dev/null
        exit 1
      fi
      WARN "NetKVM offline DriverStore copy failed; continuing because storage drivers are installed. NetKVM will be installed online by firstboot pnputil."
      printf '%s\n' "Driver.netkvm.stage=WARN_COPY_FAILED" | sudo tee -a "$REPAIR_REPORT_FILE" >/dev/null
    fi
  else
  ...
  # When MSI-installed, viostor.sys lives in DriverStore — allow absence from drivers/ dir
  if [ ! -f "$DRIVERS_DIR/viostor.sys" ] && [ "$_VIOSTOR_ALREADY_INSTALLED" -eq 0 ]; then
    ...
    if [ "$REQUIRED_IO_ERROR" -eq 1 ]; then
      invalidate_qcow_artifact
    fi
    exit 1
  fi
  printf '%s\n' "Driver.viostor.presence=OK" | sudo tee -a "$REPAIR_REPORT_FILE" >/dev/null
  if [ -f "$DRIVERS_DIR/vioscsi.sys" ] || [ "$_VIOSCSI_ALREADY_INSTALLED" -eq 1 ]; then
    printf '%s\n' "Driver.vioscsi.presence=OK" | sudo tee -a "$REPAIR_REPORT_FILE" >/dev/null
  else
    FAIL "Post-copy check failed: missing $DRIVERS_DIR/vioscsi.sys"
    printf '%s\n' "Driver.vioscsi.presence=FAIL" | sudo tee -a "$REPAIR_REPORT_FILE" >/dev/null
    if [ "$REQUIRED_IO_ERROR" -eq 1 ]; then
      invalidate_qcow_artifact
    fi
    exit 1
...
if [ $DRY_RUN -eq 0 ]; then
  backup_registry_hives || exit 1
  validate_registry_hive "$HIVE_SYSTEM" "SYSTEM" || exit 1
...
REGEOF
  ...
  # Apply registry changes atomically using a work copy.
  REG_RC=0
  run_registry_edit_atomically "$REG_FILE" || REG_RC=$?
  rm -f "$REG_FILE"

  if [ $REG_RC -eq 0 ]; then
    PASS "Registry: viostor service (Start=0, Group=SCSI miniport)"
    PASS "Registry: vioscsi service (Start=0, Group=SCSI miniport)"
    if [ "${NETKVM_PRESENT:-0}" -eq 1 ]; then
      _cfg_net="$(dirname "$HIVE_SYSTEM")"
      sudo rm -f "$_cfg_net/SYSTEM.work" "$_cfg_net/SYSTEM.pre_ospc2flex" 2>/dev/null || true
      sudo cp -a "$HIVE_SYSTEM" "$_cfg_net/SYSTEM.work"
      if ! _ensure_netkvm_service_via_hivex "$_cfg_net/SYSTEM.work"; then
        sudo rm -f "$_cfg_net/SYSTEM.work" 2>/dev/null || true
        FAIL "Registry: could not create/repair netkvm service key"
        registry_repair_abort_with_quarantine "WINDOWS_SYSTEM_REGISTRY_HIVE_CORRUPTED"
      fi
      commit_system_work_hive "$_cfg_net" || registry_repair_abort_with_quarantine "WINDOWS_SYSTEM_REGISTRY_HIVE_CORRUPTED"
      PASS "Registry: netkvm service (Start=3, Group=NDIS)"
    else
      WARN "Registry: netkvm service verification skipped (netkvm.sys not injected)"
    fi
  ...
  if [ "${NETKVM_PRESENT:-0}" -eq 1 ]; then
    ...
    WARN "Registry verification: netkvm skipped (netkvm.sys not injected)"
    printf '%s\n' "Registry.netkvm.Start=SKIPPED_NO_DRIVER" | sudo tee -a "$REPAIR_REPORT_FILE" >/dev/null
  fi
  ...
  _cfg_md="$(dirname "$HIVE_SYSTEM")"
  sudo rm -f "$_cfg_md/SYSTEM.work" "$_cfg_md/SYSTEM.pre_ospc2flex" 2>/dev/null || true
  sudo cp -a "$HIVE_SYSTEM" "$_cfg_md/SYSTEM.work"
  _hx_md_err=$(mktemp)
  set +e
  sudo hivexsh -w "$_cfg_md/SYSTEM.work" <<'EOF' 2>"$_hx_md_err"
cd \MountedDevices
setval 0
commit
EOF
  ...
  rm -f "$_hx_md_err"
  if ! validate_registry_hive "$_cfg_md/SYSTEM.work" "SYSTEM.work"; then
    sudo rm -f "$_cfg_md/SYSTEM.work" 2>/dev/null || true
    registry_repair_abort_with_quarantine "WINDOWS_SYSTEM_REGISTRY_HIVE_CORRUPTED"
  fi
  commit_system_work_hive "$_cfg_md" || registry_repair_abort_with_quarantine "WINDOWS_SYSTEM_REGISTRY_HIVE_CORRUPTED"
...
# ── Step 6b: Remove Xen class UpperFilters/LowerFilters ─────────────────────
# XENFILT registered as UpperFilter on the IDE controller class
# ({4D36E96A-E325-11CE-BFC1-08002BE10318}) causes INACCESSIBLE_BOOT_DEVICE on
# KVM even when xenfilt Start=4, because PnP loads class filters regardless of
# the service Start value.
echo ""
echo "── Step 6b: Remove Xen device class filters ───────────────────────────────"
if [ "${DRY_RUN:-0}" -eq 0 ]; then
  sudo python3 - "$HIVE_SYSTEM" <<'PY'
import sys, hivex

HIVE = sys.argv[1]
REG_MULTI_SZ = 7
XEN_DRIVERS = {"XENFILT", "XENVBD", "XENBUS", "XENPCI"}

# Device classes to scrub: IDE ({4D36E96A}), Disk ({4D36E967}), SCSI ({4D36E97B})
CLASSES = [
    "{4D36E96A-E325-11CE-BFC1-08002BE10318}",
    "{4D36E967-E325-11CE-BFC1-08002BE10318}",
    "{4D36E97B-E325-11CE-BFC1-08002BE10318}",
]
FILTER_VALUES = ["UpperFilters", "LowerFilters"]

h = hivex.Hivex(HIVE, write=True)
changed = False

def get_child(h, node, name):
    ch = h.node_get_child(node, name)
    return ch if ch else None

for cs in ("ControlSet001", "ControlSet002"):
    csn = get_child(h, h.root(), cs)
    if not csn:
        continue
    ctrl = get_child(h, csn, "Control")
    if not ctrl:
        continue
    cls_root = get_child(h, ctrl, "Class")
    if not cls_root:
        continue
    for guid in CLASSES:
        cls_node = get_child(h, cls_root, guid)
        if not cls_node:
            continue
        for fv in FILTER_VALUES:
            try:
                val = h.node_get_value(cls_node, fv)
            except RuntimeError:
                continue
            entries = h.value_multiple_strings(val)
            cleaned = [e for e in entries if e and e.upper() not in XEN_DRIVERS]
            if len(cleaned) == len([e for e in entries if e]):
                continue
            if cleaned:
                encoded = ("".join(e + "\0" for e in cleaned) + "\0").encode("utf-16-le")
                h.node_set_value(cls_node, {"key": fv, "t": REG_MULTI_SZ, "value": encoded})
                sys.stdout.write(f"  FIXED {cs}\\Control\\Class\\{guid} {fv}: removed Xen entries, kept {cleaned}\n")
            else:
                h.node_set_value(cls_node, {"key": fv, "t": REG_MULTI_SZ, "value": b"\x00\x00"})
                sys.stdout.write(f"  FIXED {cs}\\Control\\Class\\{guid} {fv}: cleared (was Xen-only)\n")
            changed = True

if changed:
    h.commit(HIVE)
    sys.stdout.write("  Committed Xen class filter removal.\n")
else:
    sys.stdout.write("  No Xen class filters found (already clean).\n")
PY
  _xf_rc=$?
  [ "$_xf_rc" -eq 0 ] && PASS "Xen device class filters removed" || WARN "Xen class filter removal returned rc=$_xf_rc"
else
  INFO "[DRY-RUN] Would remove Xen UpperFilters/LowerFilters from IDE/Disk/SCSI device classes"
fi
...
if [ $DRY_RUN -eq 0 ]; then
  _cfg_st="$(dirname "$HIVE_SYSTEM")"
  sudo rm -f "$_cfg_st/SYSTEM.work" "$_cfg_st/SYSTEM.pre_ospc2flex" 2>/dev/null || true
  sudo cp -a "$HIVE_SYSTEM" "$_cfg_st/SYSTEM.work"
  if _set_std_storage_start_via_hivex "$_cfg_st/SYSTEM.work"; then
    if validate_registry_hive "$_cfg_st/SYSTEM.work" "SYSTEM.work"; then
      commit_system_work_hive "$_cfg_st" || registry_repair_abort_with_quarantine "WINDOWS_SYSTEM_REGISTRY_HIVE_CORRUPTED"
      PASS "Microsoft storage services: Start=0 set via hivex (disk, volmgr, volsnap, partmgr, mountmgr)"
    else
      sudo rm -f "$_cfg_st/SYSTEM.work" 2>/dev/null || true
      WARN "Storage hivex patch produced an unreadable hive; leaving SYSTEM unchanged"
    fi
  else
    sudo rm -f "$_cfg_st/SYSTEM.work" 2>/dev/null || true
...
if [ $DRY_RUN -eq 0 ]; then
  ...
  if [ -n "$CLOUDBOOT_BUNDLE" ] && [ -f "$CLOUDBOOT_BUNDLE/firstboot-repair.ps1" ]; then
    sudo cp "$CLOUDBOOT_BUNDLE/firstboot-repair.ps1" "$MNT/ospc2flex_cloudboot_firstboot_repair.ps1"
    CLOUDBOOT_FIRSTBOOT_STAGED=1
    PASS "CloudJumper first-boot repair staged: C:\\ospc2flex_cloudboot_firstboot_repair.ps1"
  fi
...
# ── CloudJumper compare-derived first-boot repair (if staged) ──
try {
    if (Test-Path "C:\ospc2flex_cloudboot_firstboot_repair.ps1") {
        Write-Host "[ospc2flex] Running CloudJumper first-boot repair script..."
        & "C:\ospc2flex_cloudboot_firstboot_repair.ps1"
        Write-Host "[ospc2flex] CloudJumper first-boot repair finished"
    }
} catch {
    Write-Host "[ospc2flex] CloudJumper first-boot repair error (non-fatal): $_"
}
...
try {
    $netkvmStage = 'C:\ospc2flex\drivers\NetKVM'
...
Remove-Item -Path "C:\ospc2flex_cloudboot_firstboot_repair.ps1" -Force -ErrorAction SilentlyContinue
FIRSTBOOTEOF
...
# Write sentinel
if [ $DRY_RUN -eq 0 ]; then
  validate_registry_hive "$HIVE_SYSTEM" "SYSTEM" || {
    FAIL "Post-repair SYSTEM hive validation failed; refusing sentinel"
    registry_repair_abort_with_quarantine "WINDOWS_SYSTEM_REGISTRY_HIVE_CORRUPTED"
  }
...
echo "    Drivers: viostor (block) + vioscsi (SCSI) offline; NetKVM staging attempted (offline copy failed; will rely on first-boot/console fallback)"
fi
if [ "$DRY_RUN" -eq 1 ]; then
