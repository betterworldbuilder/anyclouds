...
# Find the largest mount point where we can actually write — skip read-only filesystems.
# NOTE: [ -w ] only checks permission bits, not filesystem mount flags (ro/rw).
# We use a real write probe (touch + rm) to detect read-only mounts like /mnt/wincheck14.
BEST_MOUNT=$(df -P -k | awk 'NR>1 && $1 !~ /tmpfs|udev|devtmpfs|overlay|shm|loop/ && $6 !~ /^[/](boot|run|dev|sys|proc|snap|var[/]lib)/ && $6 != "/" {{print $4, $6}}' | sort -rn | awk '{{print $2}}' | while read -r _mnt; do _probe="$_mnt/.ospc2flex_wtest_$$"; if touch "$_probe" 2>/dev/null; then rm -f "$_probe" 2>/dev/null; echo "$_mnt"; break; fi; done)

if [ -n "$BEST_MOUNT" ]; then
    workdir="$BEST_MOUNT/ospc2flex_image"
else
    workdir=$(eval echo {shell_quote(origin_image_dir)})
    log "[WARN] Strict policy fallback: No writable external data volumes found. Resorting to configured origin-image-dir."
fi
...
if [ -z "$OS_IMAGE_URL" ]; then
  _FALLBACK_REGION=$(echo "${{OS_REGION_NAME:-IAD}}" | tr '[:upper:]' '[:lower:]' | sed 's/[0-9]*$//')
  log "  [WARN] Catalog lookup failed — using ${{_FALLBACK_REGION}}.images.api.rackspacecloud.com fallback"
  OS_IMAGE_URL="https://${{_FALLBACK_REGION}}.images.api.rackspacecloud.com"
fi
IMG_DOWNLOAD_URL="$OS_IMAGE_URL/v2/images/{snap_id}/file"
# Snet endpoint bypasses HTTP 413 size limits on the public Rackspace Glance endpoint.
# Works when the jumphost is on the same Rackspace ServiceNet (10.x.x.x range).
_SNET_REGION=$(echo "${{OS_REGION_NAME:-IAD}}" | tr '[:upper:]' '[:lower:]' | sed 's/[0-9]*$//')
SNET_DOWNLOAD_URL="https://snet-${{_SNET_REGION}}.images.api.rackspacecloud.com/v2/images/{snap_id}/file"
...
for _osave_url in "$OS_IMAGE_URL" "https://snet-${{_SNET_REGION}}.images.api.rackspacecloud.com"; do
  ...
  rm -f "$img_path"
  if OS_IMAGE_URL="$_osave_url" openstack image save --file "$img_path" "{snap_id}" 2>/tmp/ospc_img_save_err.txt; then
    size=$(stat -c%s "$img_path" 2>/dev/null || echo 0)
    ...
    log "  [WARN] openstack image save failed via $_osave_url: $(cat /tmp/ospc_img_save_err.txt 2>/dev/null | head -2)"
    rm -f "$img_path"
  fi
done
fi
...
for attempt in $(seq 1 $export_retries); do
  ...
  # On 413 from public endpoint, immediately retry via snet before giving up this attempt
  if [ "$HTTP_STATUS" = "413" ] && [ -n "$SNET_DOWNLOAD_URL" ]; then
    log "  [INFO] HTTP 413 from public endpoint — retrying via snet: $SNET_DOWNLOAD_URL"
    rm -f "$img_path"
    HTTP_STATUS=$(curl -s -C - -L --retry 3 --retry-delay 10 --retry-max-time 180 \\
      -H "X-Auth-Token: $OS_TOKEN" \\
      -o "$img_path" \\
      --write-out '%{{http_code}}' \\
      "$SNET_DOWNLOAD_URL" 2>/dev/null || echo '000')
    size=$(stat -c%s "$img_path" 2>/dev/null || echo 0)
    log "  HTTP $HTTP_STATUS | $size bytes received (snet)"
    if [ "$size" -gt 1048576 ]; then
      log "  [OK] Download complete via snet: $size bytes"
      success=1; break
    fi
    IMG_DOWNLOAD_URL="$SNET_DOWNLOAD_URL"
  fi
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--export-retries", type=int, default=3)
...
def main() -> None:
    ...
    if args.remote_export:
        ...
        _max_ssh_retries = 4
        for _ssh_attempt in range(1, _max_ssh_retries + 1):
            try:
                run(ssh_cmd, capture=False, dry_run=args.dry_run)
                break
            except RuntimeError as _ssh_err:
                if _ssh_attempt >= _max_ssh_retries:
                    raise
                log(f"[WARN] SSH dropped on attempt {_ssh_attempt}/{_max_ssh_retries}: {_ssh_err} — retrying in 30s (script resumes from last checkpoint)...")
                time.sleep(30)
                log(f"[INFO] Reconnecting SSH to {processing_host}...")
    ...
    if args.boot_test_vm:
        ...
        # resolve_target_flavor_with_fallback and resolve_target_network_with_fallback both auto-select
        # when their inputs are empty, so no pre-flight abort is needed here.
        if args.boot_test_vm:
...
'''
            ...
            # ── Stage 7 Linux: SSH test + diagnostics ──
            log("┌" + "─" * 54 + "┐")
            log("│ STAGE 7 ── Linux VM SSH Test + Diagnostics")
            log("│ Probe OS-specific user → SSH into FLEX VM → dump state")
            log("└" + "─" * 54 + "┘")
            _os_user_map = {
                "ubuntu":   ["ubuntu"],
                "debian":   ["debian", "root"],
            ...
            _rt = repair_os_type if 'repair_os_type' in dir() else ""
            _ssh_candidates = next(
                (v for k, v in _os_user_map.items() if _rt.startswith(k)),
                [args.ssh_user],
            )
            log(f"[INFO] OS type '{_rt}' → SSH user candidates: {_ssh_candidates}")
            # Wait for SSH port to open using first candidate
            wait_for_ssh(
                key=args.ssh_key_path,
                user=_ssh_candidates[0],
                host=test_ip,
                port=args.ssh_port,
                retries=30,
                wait=20,
            ...
            # Probe each candidate until one is accepted
            import subprocess as _sp
            _active_user = None
            for _u in _ssh_candidates:
                _probe = _sp.run(
                    f"ssh -i {args.ssh_key_path} -o BatchMode=yes -o StrictHostKeyChecking=accept-new "
                    f"-o ConnectTimeout=15 -p {args.ssh_port} {_u}@{test_ip} echo ok",
                    shell=True, capture_output=True, text=True,
                )
                if _probe.returncode == 0:
                    _active_user = _u
                    log(f"[OK] SSH auth succeeded with user: {_u}")
                    break
                log(f"[INFO] User '{_u}' rejected (code={_probe.returncode}) — trying next")
            if not _active_user:
                log(f"[WARN] No SSH user from {_ssh_candidates} accepted — skipping diagnostics")
            else:
                diag_cmd = (
                ...
                ssh_cmd = (
                    ...
                    f"-p {args.ssh_port} {_active_user}@{test_ip} {shlex.quote(diag_cmd)}"
                )
                log(f"[RUN] {ssh_cmd}")
