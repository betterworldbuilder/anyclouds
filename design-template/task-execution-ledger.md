...
| 2026-07-14 | Persist and restore last Stage 3 live-scan output | 10 min | 9/10 | Added a backend latest-scan restore endpoint keyed by Business System, made Stage 3 fetch that latest persisted run on restore, and persisted rendered terminal/cards/verdict output after each poll render. Targeted backend/UI restore tests, Python compile and JS syntax passed. Full scan-appraisal suite still has unrelated pre-existing scanner verdict failures. |

| 2026-07-14 | Resolve Stage 9A FLEX IPs to OpenStack server IDs | 10 min | 9/10 | Stage 9A now prefers sourceVmId from the capture payload and, when only a FLEX target IP is available, resolves it through openstack server list --long before calling server image create. This prevents passing public IPs directly to OpenStack snapshot creation. Python compile and 17 source-capture tests passed. |

| 2026-07-14 | Use Stage 1 manual source region for Stage 9A snapshots | 10 min | 9/10 | Stage 9A now treats the Business System FLEX Region selected in Stage 1 as the snapshot source/origin region and passes it in cloud credentials for OpenStack snapshot operations, while backend still falls back without OS_REGION_NAME if Keystone rejects that catalog region. Source-capture tests and JS syntax passed. |
