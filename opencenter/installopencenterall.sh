...
# Installs openCenter CLI, Prometheus, Node Exporter, Grafana, an OpenCenter exporter,
...
OPENCENTER_REPO="${OPENCENTER_REPO:-https://github.com/opencenter-cloud/openCenter-cli.git}"
OPENCENTER_SOURCE_DIR="${OPENCENTER_SOURCE_DIR:-/opt/opencenter-cli}"
MISE_INSTALL_DIR="${MISE_INSTALL_DIR:-/opt/mise}"
...
DEBIAN_FRONTEND=noninteractive apt-get install -y \
  ca-certificates curl wget gnupg jq tar gzip git build-essential unzip python3 python3-venv python3-pip


log "Installing mise and openCenter CLI"
install -d -o "$RUN_USER" -g "$RUN_USER" -m 0755 "$MISE_INSTALL_DIR"
if [[ ! -x "$MISE_INSTALL_DIR/bin/mise" ]]; then
  sudo -u "$RUN_USER" -H env MISE_INSTALL_PATH="$MISE_INSTALL_DIR/bin/mise" sh -c 'curl -fsSL https://mise.run | sh'
fi
ln -sfn "$MISE_INSTALL_DIR/bin/mise" /usr/local/bin/mise

if [[ -d "$OPENCENTER_SOURCE_DIR/.git" ]]; then
  log "Updating existing openCenter source checkout"
  sudo -u "$RUN_USER" -H git -C "$OPENCENTER_SOURCE_DIR" fetch --all --tags --prune
  sudo -u "$RUN_USER" -H git -C "$OPENCENTER_SOURCE_DIR" pull --ff-only
else
  rm -rf "$OPENCENTER_SOURCE_DIR"
  sudo -u "$RUN_USER" -H git clone "$OPENCENTER_REPO" "$OPENCENTER_SOURCE_DIR"
fi

chown -R "$RUN_USER":"$RUN_USER" "$OPENCENTER_SOURCE_DIR" "$MISE_INSTALL_DIR"
sudo -u "$RUN_USER" -H bash -lc "cd '$OPENCENTER_SOURCE_DIR' && '$MISE_INSTALL_DIR/bin/mise' trust --yes 2>/dev/null || true; '$MISE_INSTALL_DIR/bin/mise' install && '$MISE_INSTALL_DIR/bin/mise' run build"

if [[ ! -x "$OPENCENTER_SOURCE_DIR/bin/opencenter" ]]; then
  echo "[ERROR] openCenter build did not produce $OPENCENTER_SOURCE_DIR/bin/opencenter" >&2
  exit 1
fi
install -m 0755 "$OPENCENTER_SOURCE_DIR/bin/opencenter" /usr/local/bin/opencenter
/usr/local/bin/opencenter version

install -d -o "$RUN_USER" -g "$RUN_USER" -m 0700 \
  "$RUN_HOME/.config/opencenter" \
  "$RUN_HOME/.local/state/opencenter"
...
/usr/local/bin/opencenter version >/dev/null
...
============================================================
OpenCenter CLI and monitoring installation completed successfully.

openCenter CLI:     /usr/local/bin/opencenter
openCenter source:  ${OPENCENTER_SOURCE_DIR}
...
openCenter first steps (run as ${RUN_USER}):
  opencenter version
