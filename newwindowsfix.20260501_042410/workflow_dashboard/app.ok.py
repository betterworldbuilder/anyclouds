...
def run_image_migrator():
    ...
    script_path = "/home/dzoan/OSPC2FLEX/osflex-deployer-fullmig-3.0/ospc2Flex-Image-migtool/ospc2flex_image_migrator.py"
    ...
    if req.get('ospc_username') and req.get('ospc_apikey') and not ospc_path:
        ...
        with os.fdopen(fd, 'w') as f:
            ...
            if ospc_auth_type == 'v3':
                ...
                # v2password sends passwordCredentials → Rackspace rejects API keys → 401
                # RAX-KSKEY:apiKeyCredentials is the ONLY valid method for API key auth
    ...
    if req.get('flex_username') and req.get('flex_password') and not flex_path:
        ...
        flex_region = str(req.get('flex_region', 'DFW3')).strip() or 'DFW3'
        with os.fdopen(fd, 'w') as f:
...
def agent1_run_discovery():
    ...
    def run_discovery():
        ...
        # Write servers.csv so /api/agent1/results/discovery_csv returns live scan data
        import csv as _csv
...
# ── NBD Inline Migration API ──────────────────────────────────────────────────

def _nbd_ssh_opts(ssh_key):
    key = ssh_key.replace('~', os.path.expanduser('~'))
    return ['-i', key, '-o', 'StrictHostKeyChecking=no',
            '-o', 'BatchMode=yes', '-o', 'ConnectTimeout=15']

@app.route('/api/vm_migrator/nbd/run', methods=['POST'])
def nbd_run():
    data       = request.get_json() or {}
    jh_ip      = data.get('jumphost_ip', '').strip()
    jh_user    = data.get('jumphost_user', 'ubuntu').strip()
    ssh_key    = data.get('ssh_key_path', '~/.ssh/id_rsa').strip()
    flex_creds = data.get('flex_creds', {})
    vms        = data.get('vms', [])

    if not jh_ip:
        return jsonify({'error': 'jumphost_ip is required'}), 400
    if not vms:
        return jsonify({'error': 'no VMs specified'}), 400

    key_path = ssh_key.replace('~', os.path.expanduser('~'))
    ssh_base = ['ssh'] + _nbd_ssh_opts(ssh_key) + [f'{jh_user}@{jh_ip}']
    scp_base = ['scp'] + _nbd_ssh_opts(ssh_key)

    # Write FLEX credentials file from UI inputs
    flex_sh = (
        "#!/usr/bin/env bash\n"
        f"export OS_AUTH_URL={flex_creds.get('auth_url','https://keystone.api.dfw3.rackspacecloud.com/v3/')}\n"
        "export OS_IDENTITY_API_VERSION=3\nexport OS_INTERFACE=public\n"
        f"export OS_REGION_NAME={flex_creds.get('region','DFW3')}\n"
        "export OS_AUTH_TYPE=password\n"
        f"export OS_USERNAME={flex_creds.get('username','')}\n"
        f"export OS_PASSWORD={flex_creds.get('password','')}\n"
        f"export OS_API_KEY={flex_creds.get('password','')}\n"
        "export OS_USER_DOMAIN_NAME=rackspace_cloud_domain\n"
        "export OS_PROJECT_DOMAIN_NAME=rackspace_cloud_domain\n"
        f"export OS_PROJECT_ID={flex_creds.get('project_id','')}\n"
        f"export OS_PROJECT_NAME={flex_creds.get('project_id','')}\n"
    )
    flex_tmp = '/tmp/ospc2flex_flex_ui.sh'
    with open(flex_tmp, 'w') as f:
        f.write(flex_sh)

    # SCP mig_worker_v4.sh + flex creds to jumphost
    worker_script = '/tmp/mig_worker_v4.sh'
    subprocess.run(scp_base + [worker_script, flex_tmp,
                                f'{jh_user}@{jh_ip}:/tmp/'], capture_output=True)
    subprocess.run(ssh_base + [
        'mv /tmp/ospc2flex_flex_ui.sh /tmp/ospc2flex_flex.sh; '
        'chmod +x /tmp/ospc2flex_flex.sh /tmp/mig_worker_v4.sh'
    ], capture_output=True)

    workers = []
    for vm in vms:
        label     = vm.get('label', '')
        src_ip    = vm.get('src_ip', '')
        os_type   = vm.get('os_type', 'ubuntu24')
        src_user  = vm.get('src_user', 'ubuntu')
        flex_user = vm.get('flex_user', 'ubuntu')
        nbd_dev   = f"/dev/nbd{vm.get('nbd_idx', 0)}"
        src_port  = vm.get('src_port', 10811)
        tun_port  = vm.get('tun_port', 10821)

        if not label or not src_ip:
            workers.append({'label': label, 'error': 'label and src_ip required'})
            continue

        launch_cmd = (
            f'nohup bash /tmp/mig_worker_v4.sh '
            f'{label} {src_ip} {src_user} {os_type} {flex_user} '
            f'{nbd_dev} {src_port} {tun_port} '
            f'</dev/null >/tmp/mig_{label}.log 2>&1 & echo $!'
        )
        r = subprocess.run(ssh_base + [launch_cmd], capture_output=True, text=True)
        if r.returncode == 0:
            workers.append({'label': label, 'pid': r.stdout.strip()})
        else:
            workers.append({'label': label, 'error': r.stderr.strip() or 'launch failed'})

    return jsonify({'ok': True, 'workers': workers})


@app.route('/api/vm_migrator/nbd/stream')
def nbd_stream():
    jh_ip   = request.args.get('jumphost_ip', '')
    jh_user = request.args.get('jumphost_user', 'ubuntu')
    ssh_key = request.args.get('ssh_key', '~/.ssh/id_rsa')
    label   = request.args.get('label', '')

    if not jh_ip or not label:
        return jsonify({'error': 'jumphost_ip and label required'}), 400

    def generate():
        cmd = (['ssh'] + _nbd_ssh_opts(ssh_key) +
               [f'{jh_user}@{jh_ip}', f'tail -n 50 -f /tmp/mig_{label}.log 2>/dev/null'])
        ...
        try:
            for line in iter(proc.stdout.readline, ''):
                ...
                stripped = line.rstrip()
                yield f'data: {stripped}\n\n'
                if 'SSH OK' in stripped or 'SSH FAIL' in stripped:
...
@app.route('/api/vm_migrator/nbd/status')
def nbd_status():
    jh_ip   = request.args.get('jumphost_ip', '')
    jh_user = request.args.get('jumphost_user', 'ubuntu')
    ssh_key = request.args.get('ssh_key', '~/.ssh/id_rsa')
    ...
    r = subprocess.run(
        ['ssh'] + _nbd_ssh_opts(ssh_key) + [f'{jh_user}@{jh_ip}',
         'cat /tmp/par_results_v4.txt 2>/dev/null || echo ""'],
        capture_output=True, text=True, timeout=15
    )
    return jsonify({'ok': True, 'results': r.stdout.strip()})


@app.route('/api/vm_migrator/nbd/stop', methods=['POST'])
def nbd_stop():
    data    = request.get_json() or {}
    jh_ip   = data.get('jumphost_ip', '')
    jh_user = data.get('jumphost_user', 'ubuntu')
    ssh_key = data.get('ssh_key_path', '~/.ssh/id_rsa')

    if jh_ip:
        subprocess.run(
            ['ssh'] + _nbd_ssh_opts(ssh_key) + [f'{jh_user}@{jh_ip}',
             'pkill -f mig_worker_v4.sh 2>/dev/null; pkill -f "qemu-img convert" 2>/dev/null; true'],
