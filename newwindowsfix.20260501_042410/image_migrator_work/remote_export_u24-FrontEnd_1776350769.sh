...
repaired_path="$workdir/u24-FrontEnd-snap-20260416214532-repaired.qcow2"
converted_path="$workdir/u24-FrontEnd-snap-20260416214532.qcow2"
...
img_path="$workdir/u24-FrontEnd-snap-20260416214532.img"
...
cleaned=0
SNAP_PREFIX="u24-FrontEnd-snap-20260416214532"
...
if [ "$HAS_QEMU_IMG" = "YES" ]; then
  ...
  ssh $_SSH_OPTS "$ORIGIN_VM_USER@$ORIGIN_VM_IP"     "ORIGIN_DISK=\$(for d in /dev/vda /dev/xvda /dev/sda; do [ -b \"\$d\" ] && echo \"\$d\" && break; done); echo \"[ORIGIN] Converting \$ORIGIN_DISK → qcow2 pipe...\"; sudo qemu-img convert -f raw -O qcow2 -c \"\$ORIGIN_DISK\" /dev/stdout 2>/dev/null"     > "$converted_path"
  ...
  RAW_IMG="$workdir/u24-FrontEnd-snap-20260416214532.img"
  ssh $_SSH_OPTS "$ORIGIN_VM_USER@$ORIGIN_VM_IP"     "ORIGIN_DISK=\$(for d in /dev/vda /dev/xvda /dev/sda; do [ -b \"\$d\" ] && echo \"\$d\" && break; done); echo \"[ORIGIN] Streaming \$ORIGIN_DISK...\"; sudo dd if=\"\$ORIGIN_DISK\" bs=4M status=none 2>/dev/null"     | dd of="$RAW_IMG" bs=4M conv=sparse 2>/dev/null
...
IMG_ID=$(openstack image create \
  ...
  --format value -c id \
  "ospc2flex-ubuntu24-20260416" 2>&1 || true)
