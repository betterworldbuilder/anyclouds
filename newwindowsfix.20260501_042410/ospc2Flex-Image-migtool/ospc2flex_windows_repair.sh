...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    -h|--help)
      echo "Usage: $0 --qcow2 <path> [--nbd-dev /dev/nbdX] [--force] [--dry-run] [--purge-xen|--no-purge-xen] [--debug] [--debug-log FILE] [--debug-trace]"
      exit 0 ;;
...
cleanup() {
  ...
  rm -f "$VIRTIO_ISO" 2>/dev/null || true
      ...
      while IFS=$'\t' read -r _guid _kind _has_rs _has_re _has_bsp _has_sb; do
        ...
        _bcd_elems="\\Objects\\$_guid\\Elements"
        {
          ...
          if [ "$_kind" = "W" ]; then
            ...
            # Force a single safe-mode boot so PnP storage binding can settle, then
            # first-boot script removes safeboot for normal subsequent boots.
            if [ "$_has_sb" = "1" ]; then
              echo "cd 25000080"
              echo "setval 1"
              echo "Element"
              echo "hex:3:00,00,00,00,00,00,00,00"
            else
              echo "add 25000080"
              echo "cd 25000080"
              echo "setval 1"
...
echo "      - First boot forced to Safe Mode minimal once; first-boot script removes safeboot"
