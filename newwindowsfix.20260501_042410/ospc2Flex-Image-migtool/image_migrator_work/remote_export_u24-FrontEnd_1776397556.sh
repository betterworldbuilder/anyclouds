...
repaired_path="$workdir/u24-FrontEnd-snap-20260417100756-repaired.qcow2"
converted_path="$workdir/u24-FrontEnd-snap-20260417100756.qcow2"
...
img_path="$workdir/u24-FrontEnd-snap-20260417100756.img"
...
cleaned=0
SNAP_PREFIX="u24-FrontEnd-snap-20260417100756"
...
if [ "$HAS_QEMU_IMG" = "YES" ]; then
  ...
  RAW_IMG="$workdir/u24-FrontEnd-snap-20260417100756.img"
      ...
      case "$OS_ID" in
        ...
        ubuntu)
          ...
          if [ "$OS_MAJOR_VER" = "24" ]; then
            # ── Ubuntu 24.04 custom profile — wildcard netplan (ens3/enp3s0/eth*) ──
            # FLEX NIC may be ens3 or enp3s0; use wildcard match so any en*/eth* gets DHCP.
            # cloud-init writes 50-cloud-init.yaml locked to original OSPC MAC — delete it.
            log '  [PROFILE] Ubuntu 24.04 → wildcard netplan (en*/eth* DHCP, delete 50-cloud-init.yaml)'
            sudo rm -f "$MNT/etc/netplan/50-cloud-init.yaml" 2>/dev/null || true
            sudo rm -f "$MNT/etc/netplan/50-curtin-networking.yaml" 2>/dev/null || true
            log '  [OK] Ubuntu 24: deleted MAC-locked netplan files'
...
NETPLAN_U24_EOF
            ...
            log '  [OK] Ubuntu 24: wrote 99-ospc2flex.yaml (wildcard en*/eth* DHCP)'
          else
...
NETPLAN_FALLBACK_EOF
      ...
      case "$CHROOT_PKG_MGR" in
        apt)
          ...
          # Re-write netplan after apt install — cloud-init postinstall may wipe it
          if [ "$OS_ID" = "ubuntu" ]; then
            sudo rm -f "$MNT/etc/netplan/50-cloud-init.yaml" "$MNT/etc/netplan/50-curtin-networking.yaml" 2>/dev/null || true
            sudo mkdir -p "$MNT/etc/netplan"
            sudo tee "$MNT/etc/netplan/99-ospc2flex.yaml" >/dev/null <<'NETPLAN_POSTINST_EOF'
network:
  version: 2
  renderer: networkd
  ethernets:
    ...
    all-eth:
      ...
      dhcp6: false
      optional: true
NETPLAN_POSTINST_EOF
            sudo chmod 600 "$MNT/etc/netplan/99-ospc2flex.yaml"
            log "  [OK] Ubuntu: re-wrote 99-ospc2flex.yaml after apt install (ensure cloud-init didn't wipe it)"
          fi
          ;;
        dnf|yum|zypper)
...
IMG_ID=$(openstack image create \
  ...
  --format value -c id \
  "ospc2flex-ubuntu24-20260417" 2>&1 || true)
