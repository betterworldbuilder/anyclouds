...
repaired_path="$workdir/test-dry-generic-snap-20260410132848-repaired.qcow2"
converted_path="$workdir/test-dry-generic-snap-20260410132848.qcow2"
...
img_path="$workdir/test-dry-generic-snap-20260410132848.img"
...
cleaned=0
SNAP_PREFIX="test-dry-generic-snap-20260410132848"
...
ORIGIN_VM_IP=1.2.3.4
ORIGIN_VM_USER=ubuntu
...
log "  [INFO] Streaming origin VM disk → raw image on jumphost then converting to qcow2..."
log "  [INFO] SSH pipe → raw .img first (qemu-img file driver requires a regular file, not a pipe/FIFO)"
RAW_IMG="$workdir/test-dry-generic-snap-20260410132848.img"
ssh -i "$ORIGIN_VM_KEY"     -o BatchMode=yes     -o StrictHostKeyChecking=accept-new     -o ConnectTimeout=30     -o ServerAliveInterval=30     -o ServerAliveCountMax=20     -o Compression=no     "$ORIGIN_VM_USER@$ORIGIN_VM_IP"     "ORIGIN_DISK=\$(for d in /dev/vda /dev/xvda /dev/sda; do [ -b \"\$d\" ] && echo \"\$d\" && break; done); echo \"[ORIGIN] Streaming \$ORIGIN_DISK...\"; sudo dd if=\"\$ORIGIN_DISK\" bs=4M status=progress 2>/dev/null"   > "$RAW_IMG"
...
log "  [OK] Raw disk received: $RAW_IMG ($(ls -lh "$RAW_IMG" | awk '{print $5}')), converting to qcow2..."
...
REPAIR_METHOD=generic   # custom_os | generic
if [ "$REPAIR_METHOD" = "generic" ]; then
  stage_start '4.5' 'Offline Guest Repair' 'Generic mode — skipping per-OS profile, staging for ospc2flex_offline_repair.sh (Stage 4.6)'
  log "  [INFO] Generic repair method selected — per-OS profile skipped"
  log "  [INFO] Copying image for standalone repair in Stage 4.6..."
  cp "$converted_path" "$repaired_path"
  log "  [INFO] Image staged: $repaired_path"
  stage_done '4.5'
else
...
# custom_os: runs only if Stage 4.5 repair_ok=0 (NBD fail / mount fail / unknown OS)
# generic:   always runs — Stage 4.5 left repair_ok=0 intentionally
if [ "$REPAIR_METHOD" = "generic" ] || [ $repair_ok -eq 0 ]; then
  _46_desc="Running ospc2flex_offline_repair.sh"; [ "$REPAIR_METHOD" = "generic" ] && _46_desc="Generic mode — running ospc2flex_offline_repair.sh (primary method)"
  stage_start '4.6' 'Standalone Repair Fallback' "$_46_desc"
...
IMG_ID=$(openstack image create \
  ...
  --format value -c id \
  "test-dry-generic-snap-20260410132848-flex" 2>&1 || true)
