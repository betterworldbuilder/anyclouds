...
if [ "$OS_FAMILY" = "linux" ]; then
  ...
  if [ -f "$WIN_REPAIR" ]; then
    WIN_REPAIRED_SENTINEL="${QCOW}.win_repaired"
    if [ -f "$WIN_REPAIRED_SENTINEL" ] && [ "${OSPC2FLEX_WIN_REPAIR_FORCE:-0}" != "1" ]; then
      INFO "Windows repair sentinel found: $WIN_REPAIRED_SENTINEL"
      INFO "Skipping Windows repair rerun (set OSPC2FLEX_WIN_REPAIR_FORCE=1 to force rerun)."
      step_done "SKIPPED (already repaired)"
    else
    set +e  # temporarily disable abort on error
    REPAIR_ARGS=(--qcow2 "$QCOW")
    if [ "${OSPC2FLEX_WIN_REPAIR_FORCE:-0}" = "1" ] || [ ! -f "$WIN_REPAIRED_SENTINEL" ]; then
      REPAIR_ARGS+=(--force)
    fi
    if [ "${OSPC2FLEX_WIN_PURGE_XEN:-0}" = "1" ]; then
      REPAIR_ARGS+=(--purge-xen)
      INFO "Windows repair: aggressive Xen purge enabled (OSPC2FLEX_WIN_PURGE_XEN=1)"
    else
      REPAIR_ARGS+=(--no-purge-xen)
      INFO "Windows repair: Xen safe-disable mode enabled (set OSPC2FLEX_WIN_PURGE_XEN=1 for purge)"
    fi
...
if [ "$IMG_DISK_BUS" = "ide" ]; then
  echo "$PROPS" | grep -q "hw_disk_bus='ide'\|hw_disk_bus=ide" || {
...
if [ "$IMG_DISK_BUS" = "virtio" ]; then
  echo "$PROPS" | grep -q "hw_disk_bus='virtio'\|hw_disk_bus=virtio" || {
...
if [ "$IMG_DISK_BUS" = "scsi" ]; then
  echo "$PROPS" | grep -q "hw_disk_bus='scsi'\|hw_disk_bus=scsi" || {
    FAIL "SCSI metadata assertion failed: hw_disk_bus=scsi not found"
    step_done "FAILED"
    exit 99
  }
  echo "$PROPS" | grep -q "hw_scsi_model='virtio-scsi'\|hw_scsi_model=virtio-scsi" || {
