...
# never be marked DONE from inside Flex Migration Hub — producing a plan does not
        ...
        out.append(
            {
                "step": name,
                "status": status,
                "performed_by": "Flex Migration Hub" if i < owned else "Rackspace delivery",
...
def report_markdown(project: Dict[str, Any]) -> str:
    ...
    md.append("---")
    md.append("")
    md.append("Generated by Flex Migration Hub Stage 9 — AI Adoption & Production Factory.")
    return "\n".join(md)
