...
window.r6pContent=function(n){
  ...
  if(n===5){var isSmart=R6P.captureMethod==='smart';return '<div style="display:flex;gap:16px;flex-wrap:wrap;margin-bottom:16px;"><div class="r6p-method-card'+(isSmart?' selected':'')+'" onclick="r6pSetMethod(\'smart\')"><div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px;"><strong style="font-size:15px;">Smart Snapshot Capture</strong><span style="background:#dcfce7;color:#16a34a;padding:2px 10px;border-radius:999px;font-size:10px;font-weight:800;">Recommended</span></div><p style="font-size:12px;color:#475569;line-height:1.6;margin-bottom:10px;">Creates or uses a safe snapshot, scans read-only, extracts the real application, externalizes state/config/secrets, generates clean Kubernetes/OpenCenter files.</p><div style="font-size:11px;color:#0369a1;margin-bottom:8px;">Smart Snapshot = cloud-native app extraction.</div><div><span style="background:#dcfce7;color:#16a34a;padding:2px 8px;border-radius:999px;font-size:10px;font-weight:700;">CLOUD_NATIVE_READY</span> <span style="background:#fef3c7;color:#d97706;padding:2px 8px;border-radius:999px;font-size:10px;font-weight:700;">READY_WITH_EXTERNALIZATION</span></div></div><div class="r6p-method-card compat'+(!isSmart?' selected':'')+'" onclick="r6pSetMethod(\'compat\')"><div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px;"><strong style="font-size:15px;">Full Snapshot Compatibility</strong><span style="background:#faf5ff;color:#7c3aed;padding:2px 10px;border-radius:999px;font-size:10px;font-weight:800;">Legacy Fallback</span></div><p style="font-size:12px;color:#475569;line-height:1.6;margin-bottom:8px;">Full Snapshot = legacy compatibility container. Not fully cloud-native.</p><div style="background:#fef3c7;border:1px solid #fcd34d;border-radius:6px;padding:8px;font-size:11px;color:#92400e;margin-bottom:8px;">Full Snapshot may create large images and accidentally include secrets. Use only when Smart Snapshot is not possible.</div><div><span style="background:#faf5ff;color:#7c3aed;padding:2px 8px;border-radius:999px;font-size:10px;font-weight:700;">COMPATIBILITY_CONTAINER_ONLY</span></div></div></div><div id="r6p-compat-confirm-box" style="display:none;background:#fef3c7;border:2px solid #f59e0b;border-radius:8px;padding:14px;margin-bottom:14px;"><strong style="color:#92400e;">Confirm Full Snapshot Compatibility Mode</strong><br><span style="font-size:12px;color:#78350f;">I understand this creates a compatibility container. It is NOT fully cloud-native and requires manual hardening.</span><br><br><button class="r6p-btn danger" onclick="r6pConfirmCompat()" style="margin-right:8px;">Confirm</button><button class="r6p-btn secondary" onclick="r6pSetMethod(\'smart\')">Cancel</button></div>'+r6pFoot(5);}
  if(n===6)return '<div class="r6p-info-box">Mount snapshot read-only and scan for OS, runtime, ports, services, volumes, config, secrets, dependencies.</div>'+r6pCmd('6a','hostnamectl && uname -a')+r6pCmd('6b','ss -tulpn')+r6pCmd('6c','systemctl --type=service --state=running')+r6pCmd('6d','ps aux --sort=-%mem | head -30')+r6pCmd('6e','find /mnt/snap-capture -type f -not -path "*/proc/*" | head -50')+r6pFoot(6);
