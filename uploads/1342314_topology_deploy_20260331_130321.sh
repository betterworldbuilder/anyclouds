...
create_or_reuse instance 'Windows Server 2019Re' os server create --flavor gp.5.2.4 --image 'Windows Server 2019' --network tenant-net --security-group default --password LxKjlS95xOQE5q 'Windows Server 2019Re'
create_or_reuse instance win2019websql2019 os server create --flavor gp.5.2.4 --image 'Windows Server 2019 with SQL 2019 Web' --network tenant-net --security-group default --password haLhGnAU3q5D6u win2019websql2019
create_or_reuse instance 'Windows Server 2016 + SQL Server 2019' os server create --flavor gp.5.2.4 --image 'Windows Server 2016' --network tenant-net --security-group default --password kDr6ATy9IwBALw 'Windows Server 2016 + SQL Server 2019'
...
if server_has_volume u24-postgresl data-windows-server-2016-sql-server-2019-1; then
  echo "Volume data-windows-server-2016-sql-server-2019-1 is already attached to u24-postgresl; skipping attach."
else
  ...
  if [[ "$vol_status" == "available" ]]; then
    ...
    echo "Volume data-windows-server-2016-sql-server-2019-1 is in-use and not attached to u24-postgresl; skipping attach." >&2
  else
...
if server_has_volume u24-FrontEnd data-windows-server-2016-sql-server-2019-1; then
  echo "Volume data-windows-server-2016-sql-server-2019-1 is already attached to u24-FrontEnd; skipping attach."
else
  vol_status=$(volume_status data-windows-server-2016-sql-server-2019-1)
  if [[ "$vol_status" == "available" ]]; then
    os server add volume u24-FrontEnd data-windows-server-2016-sql-server-2019-1 || true
  elif [[ "$vol_status" == "in-use" ]]; then
    echo "Volume data-windows-server-2016-sql-server-2019-1 is in-use and not attached to u24-FrontEnd; skipping attach." >&2
  else
    wait_for_volume_available data-windows-server-2016-sql-server-2019-1 || true
    os server add volume u24-FrontEnd data-windows-server-2016-sql-server-2019-1 || true
  fi
...
if server_has_volume 'Windows Server 2019Re' data-windows-server-2016-sql-server-2019-1; then
  echo "Volume data-windows-server-2016-sql-server-2019-1 is already attached to Windows Server 2019Re; skipping attach."
else
  vol_status=$(volume_status data-windows-server-2016-sql-server-2019-1)
  if [[ "$vol_status" == "available" ]]; then
    os server add volume 'Windows Server 2019Re' data-windows-server-2016-sql-server-2019-1 || true
  elif [[ "$vol_status" == "in-use" ]]; then
    echo "Volume data-windows-server-2016-sql-server-2019-1 is in-use and not attached to Windows Server 2019Re; skipping attach." >&2
  else
    wait_for_volume_available data-windows-server-2016-sql-server-2019-1 || true
    os server add volume 'Windows Server 2019Re' data-windows-server-2016-sql-server-2019-1 || true
  fi
...
if server_has_volume win2019websql2019 data-windows-server-2016-sql-server-2019-1; then
  echo "Volume data-windows-server-2016-sql-server-2019-1 is already attached to win2019websql2019; skipping attach."
else
  vol_status=$(volume_status data-windows-server-2016-sql-server-2019-1)
  if [[ "$vol_status" == "available" ]]; then
    os server add volume win2019websql2019 data-windows-server-2016-sql-server-2019-1 || true
  elif [[ "$vol_status" == "in-use" ]]; then
    echo "Volume data-windows-server-2016-sql-server-2019-1 is in-use and not attached to win2019websql2019; skipping attach." >&2
  else
    wait_for_volume_available data-windows-server-2016-sql-server-2019-1 || true
    os server add volume win2019websql2019 data-windows-server-2016-sql-server-2019-1 || true
  fi
...
if server_has_volume 'Windows Server 2016 + SQL Server 2019' data-windows-server-2016-sql-server-2019-1; then
  echo "Volume data-windows-server-2016-sql-server-2019-1 is already attached to Windows Server 2016 + SQL Server 2019; skipping attach."
else
  vol_status=$(volume_status data-windows-server-2016-sql-server-2019-1)
  if [[ "$vol_status" == "available" ]]; then
    os server add volume 'Windows Server 2016 + SQL Server 2019' data-windows-server-2016-sql-server-2019-1 || true
  elif [[ "$vol_status" == "in-use" ]]; then
    echo "Volume data-windows-server-2016-sql-server-2019-1 is in-use and not attached to Windows Server 2016 + SQL Server 2019; skipping attach." >&2
  else
    wait_for_volume_available data-windows-server-2016-sql-server-2019-1 || true
    os server add volume 'Windows Server 2016 + SQL Server 2019' data-windows-server-2016-sql-server-2019-1 || true
  fi
