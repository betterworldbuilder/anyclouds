...
lb_subnet_id=$(subnet_id_from_name tenant-subnet)
if [[ -z "$lb_subnet_id" ]]; then
  echo "Skipping load balancer backend: subnet tenant-subnet not found." >&2
else
  create_or_reuse load_balancer backend os loadbalancer create --name backend --provider ovn --vip-subnet-id "$lb_subnet_id"
  wait_for_loadbalancer_active backend
  create_or_reuse listener backend-listener os loadbalancer listener create --name backend-listener --protocol TCP --protocol-port 80 backend
  wait_for_loadbalancer_active backend
  create_or_reuse pool backend-pool os loadbalancer pool create --name backend-pool --lb-algorithm ROUND_ROBIN --listener backend-listener --protocol TCP
  wait_for_loadbalancer_active backend
fi
...
create_or_reuse instance Windows_Server_2019Re-snap-20260416211842-snapwin-safe-ide-img-snapwin-safe-ide-img-r3-f2f-DFW3-to-IAD3-snapwin-final-virtio os server create --flavor gp.5.4.4 --image 'Windows Server 2019' --network tenant-net --security-group default --password 26s4ahpIyLfhVg Windows_Server_2019Re-snap-20260416211842-snapwin-safe-ide-img-snapwin-safe-ide-img-r3-f2f-DFW3-to-IAD3-snapwin-final-virtio
) &
_WIN_PIDS+=($!)
( # Windows VM: opscwin2016-20260509-150218-snap-202605091502-snapwin-virtio-ready-img-snapwin-safe-ide-img-r3-f2f-DFW3-to-IAD3-snapwin-final-virtio
create_or_reuse instance opscwin2016-20260509-150218-snap-202605091502-snapwin-virtio-ready-img-snapwin-safe-ide-img-r3-f2f-DFW3-to-IAD3-snapwin-final-virtio os server create --flavor gp.5.4.4 --image 'Windows Server 2016' --network tenant-net --security-group default --password NTgArOIGFAA5oD opscwin2016-20260509-150218-snap-202605091502-snapwin-virtio-ready-img-snapwin-safe-ide-img-r3-f2f-DFW3-to-IAD3-snapwin-final-virtio
) &
_WIN_PIDS+=($!)
echo "  -> Provisioning Linux VMs..."
create_or_reuse instance FlexDFWjumphost os server create --flavor gp.5.8.32 --image 'Ubuntu 24.04' --network tenant-net --security-group default ${KEY_NAME:+--key-name "$KEY_NAME"} FlexDFWjumphost
create_or_reuse instance IADjumphostu24 os server create --flavor gp.5.4.16 --image 'Ubuntu 24.04' --network tenant-net --security-group default ${KEY_NAME:+--key-name "$KEY_NAME"} IADjumphostu24
...
log "     openstack floating ip create PUBLICNET | then: openstack server add floating ip FlexDFWjumphost <FIP>"
...
log "ℹ  Load balancer backend has no backend instance members; skipping member wiring."
