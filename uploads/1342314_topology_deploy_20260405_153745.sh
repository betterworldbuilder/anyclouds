...
create_or_reuse instance 'Windows Server 2019Re' os server create --flavor gp.5.2.4 --image 'Windows Server 2019' --network tenant-net --security-group default --password W7mweNKprTABwu 'Windows Server 2019Re'
) &
_WIN_PIDS+=($!)
( # Windows VM: win2019websql2019
create_or_reuse instance win2019websql2019 os server create --flavor gp.5.2.4 --image 'Windows Server 2019 with SQL 2019 Web' --network tenant-net --security-group default --password njmjhGu5ujTHnZ win2019websql2019
) &
_WIN_PIDS+=($!)
( # Windows VM: Windows Server 2016 + SQL Server 2019
create_or_reuse instance 'Windows Server 2016 + SQL Server 2019' os server create --flavor gp.5.2.4 --image 'Windows Server 2016' --network tenant-net --security-group default --password 0CzNQp1pNAmsON 'Windows Server 2016 + SQL Server 2019'
) &
_WIN_PIDS+=($!)
...
phase_banner "PHASE 6: Load Balancer Member Wiring — register server IPs as pool members, floating IPs are MANUAL" "Done — see summary below"
lb_subnet_id=$(subnet_id_from_name tenant-subnet)
if [[ -n "$lb_subnet_id" ]]; then
  ...
  if [[ -n "$member_ip" ]]; then
    ...
    echo "Could not resolve IP for u24-FrontEnd on tenant-net; skipping." >&2
  fi
fi
