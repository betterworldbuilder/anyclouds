...
phase_banner "PHASE 4: Compute — Windows VMs fired in background first (sysprep is slow), Linux VMs inline" "PHASE 5: LB Creation next"
echo "  -> Firing Windows VMs in background (sysprep takes longest)..."
_WIN_PIDS=()
( # Windows VM: Windows Server 2019Re
create_or_reuse instance 'Windows Server 2019Re' os server create --flavor gp.5.2.4 --image 'Windows Server 2019' --network tenant-net --security-group default --password A1mlvwreysR024 'Windows Server 2019Re'
) &
_WIN_PIDS+=($!)
( # Windows VM: win2019websql2019
create_or_reuse instance win2019websql2019 os server create --flavor gp.5.2.4 --image 'Windows Server 2019 with SQL 2019 Web' --network tenant-net --security-group default --password SeyZ010z5RlCAV win2019websql2019
) &
_WIN_PIDS+=($!)
( # Windows VM: Windows Server 2016 + SQL Server 2019
create_or_reuse instance 'Windows Server 2016 + SQL Server 2019' os server create --flavor gp.5.2.4 --image 'Windows Server 2016' --network tenant-net --security-group default --password rgR0zBOd35BERT 'Windows Server 2016 + SQL Server 2019'
) &
_WIN_PIDS+=($!)
...
phase_banner "PHASE 7: Volume Attachment — attach pre-created volumes to running instances (max 5 retries each)"
