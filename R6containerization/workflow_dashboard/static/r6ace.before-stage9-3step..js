...
window.r6pContent=function(n){
  ...
  if(n===9){
    ...
    +'<div style="font-size:14px;font-weight:900;color:#0f172a;margin-bottom:4px;">Stage 9A — Discover Existing VM Snapshots</div>'
    +'<div style="font-size:11px;color:#475569;margin-bottom:10px;">Scan the authenticated origin region for existing active VM snapshots, match them to approved container-source VMs, verify IDs and record reusable lineage. No new snapshots are created.</div>'
    +'<button class="r6p-btn primary" onclick="r6pGenRealDockerfiles(true)" style="padding:8px 16px;font-size:12px;">&#9654; Scan Origin Snapshots</button>'
...
window.r6pGenRealDockerfiles=function(snapshotOnly){
  ...
  if(st){st.textContent=snapshotOnly?'Scanning existing snapshots in origin region and matching lineage...':'Building containers from approved snapshot lineage...';st.style.color='#0369a1';}
  if(snapshotOnly)r6pSetStage9SnapshotLog('Stage 9A origin snapshot scan\nRegion: '+stage9Region+'\nComponents: '+workloads.map(function(w){return w.component+' -> '+(w.sourceVmId||w.targetIp||'unmapped');}).join('\n')+'\nStatus: SCANNING');
