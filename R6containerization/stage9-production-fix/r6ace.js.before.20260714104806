...
window.r6pContent=function(n){
  ...
  if(n===9){
    ...
    +'<label style="display:inline-flex;align-items:center;gap:6px;margin:8px 0 12px;font-size:11px;color:#92400e;font-weight:800;"><input id="r6p-stage9-override-unscanned" type="checkbox" checked> Override missing Full Scan evidence</label>'
    ...
    +'<div style="font-size:11px;color:#475569;margin-bottom:10px;">Reuse an approved existing origin snapshot when available; otherwise create the VM image or volume snapshot using the existing OpenStack snapshot builder, then record reusable lineage.</div>'
    +"<div style=display:flex;gap:8px;flex-wrap:wrap;align-items:center;margin-bottom:8px;><button class=r6p-btn onclick=r6pGenRealDockerfiles(true)>＋ Create Snapshot</button><button class=r6p-btn onclick=r6pGenRealDockerfiles(true)>🔎 Snapshot Scan</button></div>"
    ...
    +'</div>'
    +"<div style=background:#ffffff;border:1px solid #dbeafe;border-radius:10px;padding:12px;margin-bottom:14px;>"
    +"<div style=font-size:14px;font-weight:900;color:#0f172a;margin-bottom:4px;>Stage 9B — Extract VM Data</div>"
    +"<div style=font-size:11px;color:#475569;margin-bottom:10px;>After snapshot lineage is available, run read-only VM extraction only: application paths, secret/log/temp/host-key exclusions, checksums and lineage. No registry or build controls here.</div>"
    +"<div style=display:flex;gap:10px;flex-wrap:wrap;align-items:flex-end;margin-bottom:14px;>"
    +"<div><label>Component extraction scope</label><select id=r6p-extract-comp><option value=__all__>All approved extraction targets</option></select></div>"
    +"<div><label>SSH User</label><input id=r6p-extract-user value=ubuntu></div>"
    +"<div><label>SSH Private Key Path / Secret Ref</label><input id=r6p-extract-key value=~/.ssh/id_rsa></div>"
    +"<div><label>Managed known_hosts</label><input id=r6p-extract-known-hosts value=./data/ssh/known_hosts></div>"
    +"<button class=r6p-btn onclick=r6pGenRealDockerfiles(false,false)>▶ Extract VM Data</button>"
    +"<button class=r6p-btn disabled>Stop Extraction</button>"
    +"<button class=r6p-btn onclick=r6pCopyStage9ExtractLog()>Copy Log</button>"
    +"</div>"
    +"<div style=font-size:11px;font-weight:800;color:#334155;margin:10px 0 4px;>Verbose VM extraction output</div>"
    +"<pre id=r6p-stage9b-log class=r6p-terminal data-terminal-output style=display:block;min-height:180px;max-height:420px;overflow:auto;white-space:pre-wrap;>Waiting for Stage 9A snapshot lineage before VM extraction...</pre>"
    +"<div id=r6p-extract-status style=font-size:12px;font-weight:600;color:#64748b;margin-top:8px;line-height:1.7;></div>"
    +"</div>"
    +"<div style=background:#f8fafc;border:1px solid #bfdbfe;border-radius:10px;padding:12px;margin-bottom:14px;>"
    +"<div style=font-size:14px;font-weight:900;color:#0f172a;margin-bottom:4px;>Stage 9C — Build Containers</div>"
    +"<div style=font-size:11px;color:#475569;margin-bottom:10px;>Build from sanitized extracted context, then SBOM, scan, sign, push and record immutable registry digests.</div>"
    +"<div style=display:flex;gap:10px;flex-wrap:wrap;align-items:flex-end;margin-bottom:14px;>"
    ...
    +"<div><label>Registry URL optional</label><input id=r6p-build-regurl placeholder=registry.example.com></div>"
    +"<div><label>Project</label><input id=r6p-build-project value=flex-apps></div>"
    +"<div><label>Registry User</label><input id=r6p-build-reguser placeholder=admin></div>"
    +"<div><label>Registry Password</label><input id=r6p-build-regpass type=password></div>"
    +"</div>"
    +"<div style=background:#f8fafc;border:1px solid #e2e8f0;border-radius:8px;padding:10px;margin-bottom:14px;>"
    +"<div style=font-size:11px;font-weight:700;color:#334155;margin-bottom:6px;>Stage 9C Execution Mode</div>"
    +"<label style=margin-right:16px;><input type=radio name=r6p-build-mode value=manual checked style=margin-right:6px;>Manual - review and run commands</label>"
    +"<label><input type=radio name=r6p-build-mode value=auto style=margin-right:6px;>Automatic - build, scan and push after generation</label>"
    +"<div style=font-size:10px;color:#94a3b8;margin-top:4px;>Automatic mode pushes images to the selected registry. Use Manual for first registry run.</div>"
    +"</div>"
    +"<button class=r6p-btn onclick=r6pGenRealDockerfiles(false,true)>▶ Build Containers</button>"
...
window.r6pGenRealDockerfiles=function(snapshotOnly,autoBuildRequested){
  ...
  if(st){st.textContent=snapshotOnly?'Building/discovering VM snapshots and matching lineage...':(autoBuildRequested?'Building containers from sanitized extracted VM data...':'Preparing VM data extraction bundle from approved snapshot lineage...');st.style.color='#0369a1';}
